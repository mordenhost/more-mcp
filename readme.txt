...
Media and events release. A four-part media arc (find, process, measure, generate images), a redesigned Permissions screen with a deliberate-consent step on high-risk toggles, and a second four-part arc adding code snippets, event data, and outbound webhooks.

New media tools:

* stock_search_images: search Unsplash and Pexels for stock photos using the API keys the Instant Images plugin already stores. No new credential store; returns candidates with license and required attribution, ready to hand to wp_upload_media_from_url.
* wp_process_image: resize and/or convert an existing library image (WebP, JPEG, PNG) through WordPress's own image editor. In-place-safe: the source attachment is never modified or deleted, the result is saved as a new attachment. No external service, no key.
* AI image generation (off by default): ai_generate_image and ai_generate_alt_text call the AI provider you already configured (OpenAI or Google). Disabled behind the "Allow AI media generation" toggle because every call spends provider credits; the key is never returned, requests go only to fixed provider endpoints, and nothing is exposed until you turn it on.

New read-only status providers on the images capability:

* EWWW Image Optimizer, WP Smush, and ShortPixel: report optimization status (bytes saved, counts, average compression) beside the existing Imagify provider. Read-only; where a vendor exposes no stable public accessor for a figure, that figure is reported null rather than guessed.

New code, events, and notifications:

* Code snippets (off by default): snippet_list and snippet_get are read-only; snippet_create and snippet_update write CSS or JS only and are gated behind the "Allow code snippets" toggle, manage_options, and two-part confirmation. PHP snippet writes are refused by name (that is remote code execution), and every created snippet is saved disabled so a human must activate it. No activate, deactivate, or delete.
* The Events Calendar (read-only): tec_list_events and tec_get_event expose events with hydrated venue and organizer detail, a thin typed layer over the plugin's own template tags.
* Outbound webhooks: on a selected WordPress event (post published, comment posted, user registered, or an MCP tool call) POST a small, HMAC-signed JSON payload to a subscribed HTTPS URL. Management tools only (list, create, delete), all manage_options-gated and confirmed; the signing secret is generated server-side and shown once. No scheduler, no AI, no agent-callable "fire a webhook" tool. Delivery is SSRF-checked, non-blocking, and size-bounded. The tool_called payload carries the tool name, status, and destructive flag only, never argument values.

Changed:

* Permissions screen redesigned around one identity per plugin: each detected plugin appears once, in a WordPress tab and a Plugins tab, with the tools, writable settings, and imported abilities it contributes shown together instead of scattered across three sections. Every non-credential control now saves the moment it changes; credential fields keep their Save button.
* High-risk permission toggles (plugin and theme management, AI media generation, discovered third-party tools, code snippets) now show a deliberate-consent modal that must be acknowledged before the switch commits. Reversible scopes are deliberately not gated. The modal is a consent affordance; the server still enforces capability and toggle on every call.
