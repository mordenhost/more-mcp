 ...
 *
 * Formerly the standalone "API Reference" panel
 * (templates/admin/settings/panel-endpoints.php), which listed only the legacy
 * per-endpoint REST routes with no indication that they are the older of two
 * surfaces. That framing sent people down the wrong path: an admin reading a
 * page titled "API Reference" reasonably assumes it describes the thing they are
 * supposed to use.
 *
 * So this page now documents all three HTTP surfaces in the order they matter —
 * the MCP endpoint first, the OAuth endpoints that support it, and the legacy
 * REST routes last, explicitly labelled as legacy.
 *
...
/**
 * The legacy REST route table.
 *
 * Kept as data rather than hand-written markup so the shape stays uniform and a
 * route cannot be added with a missing method or description. Mirrors
 * includes/API/REST_Controller.php — if a route is added there, add it here.
 */
