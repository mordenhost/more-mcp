		...
		<p>
			...
			echo wp_kses(
				sprintf(
					...
					__( 'First check that the master switch is on. <a href="%s">Access</a> shows the current state, and while it is off the server answers discovery but refuses everything else. If it is on, the next suspect is tool-list size: some clients silently drop a list they consider too large.', 'more-mcp' ),
	...
	<div class="mmcp-troubleshoot-item">
		...
		<p class="mmcp-troubleshoot-fix">
			...
			echo wp_kses(
				sprintf(
					/* translators: %s: URL of the Access panel */
					__( 'Check the relevant toggle on <a href="%s">Access</a> first; it is the more common cause. If the toggle is already on, re-authorize the connector as a user who holds the capability the operation needs.', 'more-mcp' ),
					esc_url( add_query_arg( 'panel', 'permissions', $more_mcp_ts_base ) )
				),
				[ 'a' => [ 'href' => [] ] ]
			);
