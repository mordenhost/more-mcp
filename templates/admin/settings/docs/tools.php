<?php
/**
 * Documentation → What agents can do.
 *
 * A live inventory of the MCP tool surface, read from the actual registry rather
 * than a hand-maintained list. Two reasons for that: the surface changes with
 * which third-party plugins are active on this specific site, and a hardcoded
 * list would drift the first time a tool was added without someone remembering
 * to edit a docs template.
 *
 * The Permissions panel controls what agents may WRITE; this page answers the
 * question that comes just before it — what is exposed at all.
 *
...
/**
 * Tool-name prefix → display group.
 *
 * Longest prefix wins, so `more_mcp_` must be tested before `wp_`. This mirrors
 * the ordering constraint documented in Abilities\Registrar::prefix_to_category()
...
$more_mcp_tool_groups = [
	...
	'gp_'          => [
		'label'   => __( 'GuardPress', 'more-mcp' ),
		'summary' => __( 'Security posture reads. Present only while GuardPress is active.', 'more-mcp' ),
	],
	'sv_'          => [
		'label'   => __( 'SiteVault', 'more-mcp' ),
		'summary' => __( 'Backup status reads. Present only while SiteVault is active.', 'more-mcp' ),
	],
	'fc_'          => [
		'label'   => __( 'ForgeCache', 'more-mcp' ),
		'summary' => __( 'Cache status and purges. Present only while ForgeCache is active.', 'more-mcp' ),
	],
	'wp_'          => [
 ...
 *
 * Guarded rather than assumed: this template is included from a settings screen
 * that must render even if the MCP subsystem failed to autoload, and an empty
 * inventory with an explanatory notice is a better failure than a fatal on the
 * settings page.
 */
$more_mcp_all_tools = [];
...
// Bucket each tool into the first matching prefix. Anything unmatched lands in a
// catch-all group rather than being dropped — a tool added by a third-party
// filter under an unknown prefix should still be visible here.
$more_mcp_bucketed = [];
$more_mcp_other    = [];
...
// Lifecycle tools deliberately share the plain wp_ prefix (they are core
// WordPress operations), so they cannot be split out by prefix. Their presence
// is worth calling out separately because it is the one group whose visibility
// is a setting rather than a plugin detection.
	...
	<div class="mmcp-tool-groups">
		...
		foreach ( $more_mcp_tool_groups as $more_mcp_prefix => $more_mcp_group ) :
			...
			if ( empty( $more_mcp_group_tools ) ) {
				// An integration that is not installed contributes nothing. Skip it
				// entirely rather than showing an empty group — a list of things the
				// site does not have is noise on the page that answers "what do I have".
				continue;
			}
			?>
						...
						<?php foreach ( $more_mcp_group_tools as $more_mcp_group_tool ) : ?>
							<li>
								...
								<span class="mmcp-tool-desc">
									<?php
									// Descriptions are long by design — they are prompt material for
									// the model. Trim for the list; the full text is what the client
									// receives regardless of what is shown here.
