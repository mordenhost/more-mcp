...
$more_mcp_tool_groups = [
	...
	'gsc_'         => [
		'label'   => __( 'Google Search Console', 'more-mcp' ),
		'summary' => __( 'Search analytics, sitemaps, and URL inspection. Present once a service account key is stored on External Services.', 'more-mcp' ),
	],
	'ga4_'         => [
		'label'   => __( 'Google Analytics 4', 'more-mcp' ),
		'summary' => __( 'Reports, realtime, and property metadata. Present once a service account key is stored on External Services.', 'more-mcp' ),
	],
	'semrush_'     => [
		'label'   => __( 'Semrush', 'more-mcp' ),
		'summary' => __( 'Domain and keyword research, competitors, and backlinks. Present once an API key is stored on External Services.', 'more-mcp' ),
	],
	'dataforseo_'  => [
		'label'   => __( 'DataForSEO', 'more-mcp' ),
		'summary' => __( 'Live SERPs, keyword volume, ranked keywords, and backlink data. Present once API credentials are stored on External Services.', 'more-mcp' ),
	],
	'seranking_'   => [
		'label'   => __( 'SE Ranking', 'more-mcp' ),
		'summary' => __( 'Domain overview, keyword and competitor research. Present once an API token is stored on External Services.', 'more-mcp' ),
	],
	'ahrefs_'      => [
		'label'   => __( 'Ahrefs', 'more-mcp' ),
		'summary' => __( 'Domain Rating. Present once an Ahrefs APIv3 key is stored on External Services.', 'more-mcp' ),
	],
	'wpr_'         => [
	...
	'discovered_'  => [
		'label'   => __( 'Discovered (imported abilities)', 'more-mcp' ),
		'summary' => __( 'Abilities other plugins registered with the WordPress Abilities API, wrapped as tools. Present only for abilities you explicitly enabled on the Access panel; they run the other plugin\'s own code and carry none of More MCP\'s dry-run / undo guards.', 'more-mcp' ),
	],
	'wp_'          => [
...
<div class="mmcp-doc-lead">
	<p>
		...
		printf(
			...
			esc_html__( 'This site currently exposes %s abilities. The list is built from the live registry, so it reflects which optional integrations are active right now. It is what an AI client actually receives from tools/list.', 'more-mcp' ),
			'<strong>' . esc_html( number_format_i18n( $more_mcp_total_tools ) ) . '</strong>'
		);
		?>
	</p>
	<p class="description">
		<?php esc_html_e( 'Being listed is not the same as being permitted. Every write still runs the calling user\'s WordPress capability check, and the option, theme, and plugin-management toggles on the Access panel gate whole categories on top of that.', 'more-mcp' ); ?>
	</p>
</div>
					...
					<?php if ( 'wp_' === $more_mcp_prefix ) : ?>
						<p class="mmcp-tool-group-note">
							...
							echo $more_mcp_lifecycle_on
								? esc_html__( 'Plugin and theme lifecycle tools are included in this group because installing or activating a plugin is a core WordPress operation. They are listed because you enabled plugin management on the Access panel.', 'more-mcp' )
								: esc_html__( 'Plugin and theme lifecycle tools would also appear in this group, but plugin management is off on the Access panel, so they are not listed to clients at all.', 'more-mcp' );
							?>
						</p>
					<?php endif; ?>
