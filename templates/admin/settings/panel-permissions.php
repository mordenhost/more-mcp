...
$more_mcp_dep_ai_providers    = class_exists( '\\More_MCP\\Integrations\\AIMedia' )
	...
	: false;
$more_mcp_dep_abilities_api   = function_exists( 'wp_get_abilities' );
...
require __DIR__ . '/_capability-summary.php';
?>
...
<?php if ( 'wordpress' === $more_mcp_perm_view ) : ?>
	...
	<div class="mmcp-perm-group">
		...
		<!-- Discovered tools master (WordPress-level scope, save-on-change).
		     The per-ability picker moved to the Plugins sub-tab: each plugin that
		     registers WP-Abilities appears as its own card there, with its abilities
		     enabled individually. This switch is the master gate — while it is off,
		     no discovered ability is exposed regardless of the per-plugin ticks. -->
		<div class="mmcp-scope mmcp-scope-critical <?php echo $more_mcp_perm_discovered ? 'is-on' : ''; ?>">
			<div class="mmcp-scope-head">
				<label class="switch small">
					<input type="checkbox"
					       class="mmcp-scope-toggle"
					       data-scope="allow_discovered_tools"
					       data-security-ack="<?php esc_attr_e( 'This lets an agent call abilities other plugins registered with WordPress. They run that plugin\'s own code, which More MCP has not reviewed.', 'more-mcp' ); ?>"
				...
				<div class="mmcp-scope-title">
					<h4><label for="allow_discovered_tools"><?php esc_html_e( 'Discovered abilities (WP Abilities API)', 'more-mcp' ); ?></label></h4>
					<span class="mmcp-scope-risk mmcp-risk-high"><?php esc_html_e( 'Third-party code paths', 'more-mcp' ); ?></span>
				</div>
			</div>
			<div class="mmcp-scope-body">
				<p class="description">
					<strong><?php esc_html_e( 'Master switch for abilities other plugins registered with WordPress.', 'more-mcp' ); ?></strong>
					<?php esc_html_e( 'WordPress 6.9 lets a plugin publish machine-readable "abilities". With this on, each plugin that registers one appears as its own card under the Plugins tab, where you enable its abilities individually. They run the other plugin\'s own code, which More MCP has not reviewed, and carry none of our dry-run or undo guards.', 'more-mcp' ); ?>
				</p>
				<p class="description">
					<?php esc_html_e( 'While this is off, no discovered ability is exposed, even if a plugin card has some ticked.', 'more-mcp' ); ?>
				</p>
				<?php  ?>
				<?php $more_mcp_wp_version = (string) get_bloginfo( 'version' ); ?>
				<?php if ( ! $more_mcp_dep_abilities_api ) : ?>
					<?php if ( $more_mcp_perm_discovered ) : ?>
						<p class="description mmcp-dep-note">
							<span class="mmcp-flag mmcp-flag-warn"><?php esc_html_e( 'On but unavailable here', 'more-mcp' ); ?></span>
							<?php echo wp_kses(
								sprintf(
									/* translators: %s: current WordPress version */
									__( 'This switch is on, but this site runs WordPress %s and the Abilities API needs 6.9+, so no discovered tools are exposed. Update WordPress to make it effective.', 'more-mcp' ),
									esc_html( $more_mcp_wp_version )
								),
								array()
							); ?>
						</p>
					<?php else : ?>
						<p class="description">
							<em><?php echo wp_kses(
								sprintf(
									/* translators: %s: current WordPress version */
									__( 'Requires WordPress 6.9+ for the Abilities API; this site runs %s. Discovered tools stay hidden either way until you switch this on.', 'more-mcp' ),
									esc_html( $more_mcp_wp_version )
								),
								array()
							); ?></em>
