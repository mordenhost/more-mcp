...
$more_mcp_perm_discovered = ! empty( $more_mcp_settings['allow_discovered_tools'] );

$more_mcp_importable_abilities = class_exists( '\More_MCP\Abilities\Importer' )
...
$more_mcp_integration_catalog = class_exists( '\More_MCP\Capabilities\Toggles' )
	? \More_MCP\Capabilities\Toggles::catalog()
	: [];
$more_mcp_integration_available = class_exists( '\More_MCP\Capabilities\Toggles' )
	? \More_MCP\Capabilities\Toggles::availability()
	: [];
$more_mcp_integration_enabled = class_exists( '\More_MCP\Capabilities\Toggles' )
	? array_flip( \More_MCP\Capabilities\Toggles::enabled_slugs() )
	: [];
...
$more_mcp_open_scopes = (int) $more_mcp_perm_options + (int) $more_mcp_perm_theme + (int) $more_mcp_perm_lifecycle + (int) $more_mcp_perm_discovered;
?>
...
<div class="mmcp-section">
	...
	<!-- Discovered tools (imported abilities) -->
	<div class="mmcp-scope mmcp-scope-critical <?php echo $more_mcp_perm_discovered ? 'is-on' : ''; ?>">
		<div class="mmcp-scope-head">
			<label class="switch small">
				<input type="checkbox"
				       name="more_mcp_settings[allow_discovered_tools]"
				       id="allow_discovered_tools"
				       value="1"
				       <?php checked( $more_mcp_perm_discovered ); ?>>
				<span class="slider"></span>
			</label>
			<div class="mmcp-scope-title">
				<h4><label for="allow_discovered_tools"><?php esc_html_e( 'Discovered tools from other plugins', 'more-mcp' ); ?></label></h4>
				<span class="mmcp-scope-risk mmcp-risk-high"><?php esc_html_e( 'Third-party code paths', 'more-mcp' ); ?></span>
			</div>
		</div>
		<div class="mmcp-scope-body">
			<p class="description">
				<strong><?php esc_html_e( 'Exposes abilities that other plugins registered with WordPress.', 'more-mcp' ); ?></strong>
				<?php esc_html_e( 'WordPress 6.9 lets a plugin publish machine-readable "abilities". When this is on, More MCP can wrap ones you explicitly pick as tools an agent can call. They run the other plugin\'s own code, which More MCP has not reviewed, so nothing is exposed until you tick it below.', 'more-mcp' ); ?>
			</p>
			<ul class="mmcp-doc-facts is-compact">
				<li>
					<strong><?php esc_html_e( 'Nothing auto-exposed', 'more-mcp' ); ?></strong>
					<?php esc_html_e( 'Only abilities you tick are offered. Turning the scope on with nothing ticked exposes nothing.', 'more-mcp' ); ?>
				</li>
				<li>
					<strong><?php esc_html_e( 'Permission still enforced', 'more-mcp' ); ?></strong>
					<?php esc_html_e( 'Each imported ability runs its own permission check on every call, so ticking it widens what is offered, never what a given user may do.', 'more-mcp' ); ?>
				</li>
				<li>
					<strong><?php esc_html_e( 'No More MCP safety rails', 'more-mcp' ); ?></strong>
					<?php esc_html_e( 'Imported abilities have no dry-run, undo, or the guards our native tools carry. Enable writes with care.', 'more-mcp' ); ?>
				</li>
			</ul>
			<?php require __DIR__ . '/_discovered-abilities-list.php'; ?>
		</div>
	</div>

	<!-- Third-party integrations (per-integration opt-in, saved on change) -->
	<div class="mmcp-scope">
		<div class="mmcp-scope-head">
			<div class="mmcp-scope-title">
				<h4><?php esc_html_e( 'Third-party integrations', 'more-mcp' ); ?></h4>
				<span class="mmcp-scope-risk"><?php esc_html_e( 'Opt-in per plugin', 'more-mcp' ); ?></span>
			</div>
		</div>
		<div class="mmcp-scope-body">
			<p class="description">
				<strong><?php esc_html_e( 'Each integration is off until you turn it on here.', 'more-mcp' ); ?></strong>
				<?php esc_html_e( 'When a supported plugin (WooCommerce, Elementor, a forms plugin, and so on) is active, More MCP can expose tools for it. Those tools stay hidden from connected clients until you enable the integration below. Toggling saves immediately, no Save button. Turning one off hides its tools even though the plugin itself keeps running.', 'more-mcp' ); ?>
			</p>
			<?php require __DIR__ . '/_integration-toggles.php'; ?>
		</div>
	</div>
