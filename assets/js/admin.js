jQuery(document).ready(function($) {
    ...
    // Per-integration Tools toggle (WooCommerce, Elementor, ...) inside a plugin card.
    $(document).on('change', '.mmcp-integration-toggle', function() {
        var $t = $(this);
        mmcpSaveToggle($t, {
            action: 'more_mcp_toggle_integration',
            slug: $t.data('slug'),
    ...
    // One imported ability. After saving, re-sync the namespace toggle so it
    // reflects all-on / partial / all-off without a reload.
    $(document).on('change', '.mmcp-ability-toggle', function() {
        var $t = $(this);
        mmcpSaveToggle($t, {
            action: 'more_mcp_toggle_ability',
            ability: $t.data('ability'),
            enabled: $t.is(':checked') ? '1' : '0'
        });
        mmcpSyncAbilityNamespace($t.data('namespace'));
    });

    // A whole plugin's abilities, toggled together (one switch per plugin, not per
    // ability — the SCF case: 48 abilities under one namespace, one toggle). Posts
    // the namespace; the server enables/disables every ability the plugin
    // registered under it. The per-ability switches below are updated in place.
    $(document).on('change', '.mmcp-ability-ns-toggle', function() {
        var $t  = $(this);
        var ns  = $t.data('namespace');
        var on  = $t.is(':checked');
        // Reflect the change onto every child switch immediately for responsiveness;
        // the server is the source of truth and the row reverts on failure.
        $('.mmcp-ability-toggle[data-namespace="' + mmcpAttrEsc(ns) + '"]').prop('checked', on);
        mmcpSaveToggle($t, {
            action: 'more_mcp_toggle_ability_namespace',
            namespace: ns,
            enabled: on ? '1' : '0'
        });
    });
    ...
    // Recompute a namespace toggle's checked state from its children: checked only
    // when every child is on (all-or-nothing, mirroring Tools/Settings).
    function mmcpSyncAbilityNamespace(ns) {
        if (! ns) { return; }
        var $children = $('.mmcp-ability-toggle[data-namespace="' + mmcpAttrEsc(ns) + '"]');
        if (! $children.length) { return; }
        var allOn = $children.length === $children.filter(':checked').length;
        $('.mmcp-ability-ns-toggle[data-namespace="' + mmcpAttrEsc(ns) + '"]').prop('checked', allOn);
    }
