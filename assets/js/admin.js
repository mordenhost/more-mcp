jQuery(document).ready(function($) {
    ...
    // A whole plugin's imported abilities, toggled together. This is the MASTER
    // toggle of an import-only card (the switch in the card header): one
    // decision per plugin — on exposes every ability under the namespace,
    // all-or-nothing, no per-ability picker. The disclosure below the row is
    // read-only, so there are no child switches to sync.
