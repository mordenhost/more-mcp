jQuery(document).ready(function($) {
    ...
    // --- Per-integration toggles (save-on-change) ---
    // Each switch in the Permissions panel's "Third-party integrations" list
    // posts its new state immediately; there is no Save button for this control.
    // On success the row's status label is updated in place. On failure the
    // checkbox is reverted so the UI never claims a state the server did not store.
    $(document).on('change', '.mmcp-integration-toggle', function() {
        var $toggle = $(this);
        var slug    = $toggle.data('slug');
        var enabled = $toggle.is(':checked');
        var $row    = $toggle.closest('.mmcp-integration-row');
        var $status = $row.find('.mmcp-integration-status');
        var $hint   = $('.mmcp-integration-hint');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response && response.success) {
                    if ($status.length) {
                        $status.text(enabled ? 'Enabled — tools exposed' : 'Installed — tools hidden');
                    }
                    if ($hint.length && response.data && response.data.message) {
                        $hint.text(response.data.message);
                    }
                } else {
                    // Revert the visual state to match what the server actually holds.
                    $toggle.prop('checked', !enabled);
                    showNotice((response && response.data && response.data.message) || 'Failed to update integration.', 'error');
                }
            },
            error: function() {
                $toggle.prop('disabled', false).prop('checked', !enabled);
                showNotice('Failed to update integration. Please try again.', 'error');
            }
        });
    });
});
