...
class Settings_Page {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_more_mcp_get_api_key', [$this, 'ajax_get_api_key']);
        ...
        add_action('wp_ajax_more_mcp_toggle_option_source', [$this, 'ajax_toggle_option_source']);
        add_action('wp_ajax_more_mcp_toggle_ability_namespace', [$this, 'ajax_toggle_ability_namespace']);
        add_action('wp_ajax_more_mcp_set_scope', [$this, 'ajax_set_scope']);
        add_action('wp_ajax_more_mcp_set_custom_options', [$this, 'ajax_set_custom_options']);
        add_action('wp_ajax_more_mcp_set_ttl', [$this, 'ajax_set_ttl']);
        
        
        
        add_action('wp_ajax_more_mcp_run_undo', [$this, 'ajax_run_undo']);
    }
    ...
    public function add_menu_page() {
        ...
        $more_mcp_panel_submenus = [
            'permissions' => __('Access', 'more-mcp'),
    ...
    public function sanitize_settings($input) {
        ...
        $sanitized['allow_ai_media'] = isset($input['allow_ai_media']) ? (bool) $input['allow_ai_media'] : false;
        
        
        $sanitized['allow_code_snippets'] = isset($input['allow_code_snippets']) ? (bool) $input['allow_code_snippets'] : false;
        ...
        if (isset($input[\More_MCP\Abilities\Importer::ENABLED_KEY]) && is_array($input[\More_MCP\Abilities\Importer::ENABLED_KEY])) {
            $live_ns = array_flip(\More_MCP\Abilities\Importer::namespaces_of_importable());
            foreach ($input[\More_MCP\Abilities\Importer::ENABLED_KEY] as $posted_ns) {
                if (!is_string($posted_ns)) {
                    continue;
                }
                $posted_ns = sanitize_key(trim($posted_ns));
                
                if ('' !== $posted_ns && isset($live_ns[$posted_ns])) {
                    $enabled_namespaces[] = $posted_ns;
                }
            }
        }
        $sanitized[\More_MCP\Abilities\Importer::ENABLED_KEY] = array_values(array_unique($enabled_namespaces));
        ...
        if (isset($input['writable_options_admin'])) {
            ...
            $raw = $input['writable_options_admin'];
            $lines = is_array($raw) ? $raw : preg_split('/\r?\n/', (string) $raw);
            foreach ((array) $lines as $line) {
                $clean = sanitize_key(trim((string) $line));
                if ($clean !== '') { $keys[] = $clean; }
            }
        }
    ...
    public function enqueue_scripts($hook) {
        ...
        $_more_mcp_settings_cache = get_option('more_mcp_settings', []);
        $more_mcp_api_preview_js  = \More_MCP\Auth\Api_Key::mask(
            (string) ( is_array($_more_mcp_settings_cache) ? ($_more_mcp_settings_cache['api_key'] ?? '') : '' )
        );
        ...
        wp_localize_script('more-mcp-admin', 'moreMcp', [
            ...
            'strings' => [
                ...
                'apiKeyPreview'     => $more_mcp_api_preview_js,
                'keyRevealFailed'   => esc_html__('Could not load the API key. Reload the page and try again.', 'more-mcp'),
                
                
                
                
                'revealGuideKey'    => esc_html__('Reveal key in this example', 'more-mcp'),
                'hideGuideKey'      => esc_html__('Hide key', 'more-mcp'),
                
                
                
                'ackTitle'   => esc_html__('Enable a security-sensitive permission?', 'more-mcp'),
                'ackConfirm' => esc_html__('I understand — enable it', 'more-mcp'),
                'ackCancel'  => esc_html__('Cancel', 'more-mcp'),
                'ackCheckbox' => esc_html__('I understand the risk and am enabling this deliberately.', 'more-mcp'),
            ],
        ]);
    }
    ...
    public function render_settings_page() {
        ...
        $more_mcp_api_preview = \More_MCP\Auth\Api_Key::mask( (string) ( $settings['api_key'] ?? '' ) );
    ...
    public function render_logs_page() {
        ...
        $undo_snapshots = class_exists('\More_MCP\MCP\Undo_Store')
            ? \More_MCP\MCP\Undo_Store::list_active()
            : [];
    ...
    public function ajax_toggle_integration() {
        ...
        if (class_exists('\More_MCP\Admin\Option_Presets')) {
            ...
            if (isset($summaries[$slug])) {
                $opt_current = isset($settings['writable_options_admin']) && is_array($settings['writable_options_admin'])
                    ? $settings['writable_options_admin']
                    : [];
                $opt_set = [];
                foreach ($opt_current as $name) {
                    $name = sanitize_key(trim((string) $name));
                    if ('' !== $name && !in_array($name, $summaries[$slug]['names'], true)) {
                        $opt_set[$name] = true;
                    }
                }
                if ($enabled) {
                    foreach ($summaries[$slug]['names'] as $name) {
                        $clean = sanitize_key(trim((string) $name));
                        if ('' !== $clean) {
                            $opt_set[$clean] = true;
                        }
                ...
                $settings['writable_options_admin'] = array_values(array_keys($opt_set));
            }
        }
        if (class_exists('\More_MCP\Abilities\Importer')
            && in_array($slug, \More_MCP\Abilities\Importer::namespaces_of_importable(), true)) {
            $ns_key     = \More_MCP\Abilities\Importer::ENABLED_KEY;
            $ns_current = isset($settings[$ns_key]) && is_array($settings[$ns_key]) ? $settings[$ns_key] : [];
            $ns_set = [];
            foreach ($ns_current as $n) {
                $n = is_string($n) ? trim($n) : '';
                
                if ('' !== $n && false === strpos($n, '/') && $n !== $slug) {
                    $ns_set[$n] = true;
                }
            }
            if ($enabled) {
            ...
            $settings[$ns_key] = array_values(array_keys($ns_set));
        }
    ...
    public function ajax_toggle_option_source() {
        check_ajax_referer('more_mcp_nonce', 'nonce');
        ...
        $summaries = Option_Presets::source_summaries();
        if ('' === $slug || !isset($summaries[$slug])) {
            wp_send_json_error(['message' => esc_html__('Unknown settings source.', 'more-mcp')]);
        }
        ...
        foreach ($summaries[$slug]['names'] as $name) {
            $clean = sanitize_key(trim((string) $name));
            if ('' !== $clean) {
                $source_names[$clean] = true;
            }
        }

        $settings = get_option('more_mcp_settings', []);
        $current  = isset($settings['writable_options_admin']) && is_array($settings['writable_options_admin'])
            ? $settings['writable_options_admin']
            : [];
        ...
        foreach ($current as $name) {
            $name = sanitize_key(trim((string) $name));
            if ('' !== $name && !isset($source_names[$name])) {
                $kept[$name] = true;
            }
        }
        if ($enabled) {
            foreach ($source_names as $name => $_) {
                $kept[$name] = true;
            }
        }
        $settings['writable_options_admin'] = array_values(array_keys($kept));

        update_option('more_mcp_settings', $settings);

        $this->log_admin_action(
            'option_source:toggle',
            ['slug' => $slug, 'enabled' => $enabled],
            ['allowlist_count' => count($settings['writable_options_admin'])]
        );

        wp_send_json_success([
            'slug'    => $slug,
            'enabled' => $enabled,
            'count'   => count($settings['writable_options_admin']),
            'message' => $enabled
                ? sprintf(
                    /* translators: %s: product/source label */
                    esc_html__('%s settings are now writable by agents.', 'more-mcp'),
                    $summaries[$slug]['label']
                )
                : sprintf(
                    /* translators: %s: product/source label */
                    esc_html__('%s settings are no longer writable.', 'more-mcp'),
                    $summaries[$slug]['label']
                ),
        ]);
    }
    ...
    public function ajax_toggle_ability_namespace() {
        check_ajax_referer('more_mcp_nonce', 'nonce');
        ...
        $namespace = isset($_POST['namespace']) ? sanitize_key(wp_unslash($_POST['namespace'])) : '';
        $enabled   = isset($_POST['enabled']) && ('1' === (string) $_POST['enabled'] || 'true' === $_POST['enabled']);

        $core_ns = class_exists('\More_MCP\Capabilities\Plugin_Catalog')
            ? \More_MCP\Capabilities\Plugin_Catalog::CORE_ABILITY_NAMESPACES
            : ['core', 'wp', 'wordpress'];
        if ('' === $namespace || in_array($namespace, $core_ns, true)) {
            wp_send_json_error(['message' => esc_html__('Unknown ability group.', 'more-mcp')]);
        }

        
        
        $live_ns = class_exists('\More_MCP\Abilities\Importer')
            ? \More_MCP\Abilities\Importer::namespaces_of_importable()
            : [];
        if (!in_array($namespace, $live_ns, true)) {
            wp_send_json_error(['message' => esc_html__('No abilities found for that plugin.', 'more-mcp')]);
        }
        ...
        $current  = isset($settings[$key]) && is_array($settings[$key]) ? $settings[$key] : [];

        $set = [];
        foreach ($current as $n) {
            $n = is_string($n) ? trim($n) : '';
            if ('' !== $n && false === strpos($n, '/') && $n !== $namespace) {
                $set[$n] = true;
            }
        }
        if ($enabled) {
        ...
        $settings[$key] = array_values(array_keys($set));

        update_option('more_mcp_settings', $settings);

        $this->log_admin_action(
            'ability_namespace:toggle',
            ['namespace' => $namespace, 'enabled' => $enabled],
            ['enabled_count' => count($settings[$key])]
        );

        wp_send_json_success([
            'namespace' => $namespace,
            'enabled'   => $enabled,
            'count'     => count($settings[$key]),
        ]);
    }
    ...
    public function ajax_set_scope() {
        ...
        $allowed = ['enabled', 'allow_option_writes', 'allow_theme_writes', 'allow_plugin_management', 'allow_ai_media', 'allow_code_snippets', 'allow_discovered_tools'];
        $key     = isset($_POST['scope']) ? sanitize_key(wp_unslash($_POST['scope'])) : '';
        $enabled = isset($_POST['enabled']) && ('1' === (string) $_POST['enabled'] || 'true' === $_POST['enabled']);

        if (!in_array($key, $allowed, true)) {
            wp_send_json_error(['message' => esc_html__('Unknown scope.', 'more-mcp')]);
        }
        ...
        $this->log_admin_action('scope:set', ['scope' => $key, 'enabled' => $enabled]);

        wp_send_json_success(['scope' => $key, 'enabled' => $enabled]);
    }

    
    public function ajax_set_custom_options() {
        check_ajax_referer('more_mcp_nonce', 'nonce');
        ...
        $raw   = isset($_POST['names']) ? (string) wp_unslash($_POST['names']) : '';
        $lines = preg_split('/\r?\n/', $raw);
        $typed = [];
        ...
        $source_owned = [];
        foreach (Option_Presets::source_summaries() as $summary) {
            foreach ($summary['names'] as $name) {
                $clean = sanitize_key(trim((string) $name));
                if ('' !== $clean) {
                    $source_owned[$clean] = true;
                }
            }
        }
        ...
        foreach ($current as $name) {
            $name = sanitize_key(trim((string) $name));
            if ('' !== $name && isset($source_owned[$name])) {
                $kept[$name] = true;
            }
        }
        
        
        foreach ($typed as $name => $_) {
            if (!isset($source_owned[$name])) {
                $kept[$name] = true;
            }
        }
        $settings['writable_options_admin'] = array_values(array_keys($kept));
        ...
        $this->log_admin_action(
            'custom_options:set',
            ['typed_count' => count($typed)],
            ['allowlist_count' => count($settings['writable_options_admin'])]
        );

        wp_send_json_success(['count' => count($settings['writable_options_admin'])]);
    }
    ...
    public function ajax_set_ttl() {
        ...
        $posted = isset($_POST['ttl']) ? (int) $_POST['ttl'] : 0;
        $ttl    = in_array($posted, \More_MCP\OAuth\Token_Store::ACCESS_TOKEN_TTL_CHOICES, true)
            ? $posted
            : \More_MCP\OAuth\Token_Store::ACCESS_TOKEN_TTL;
        ...
        $this->log_admin_action('ttl:set', ['ttl' => $ttl]);

        wp_send_json_success(['ttl' => $ttl]);
    }

    
    public function ajax_run_undo() {
        check_ajax_referer('more_mcp_nonce', 'nonce');
        ...
        if (!preg_match('/^[a-f0-9]{32}$/', $token)) {
            wp_send_json_error(['message' => esc_html__('That undo token is not valid.', 'more-mcp')]);
        }

        
        
        
        $snapshot = \More_MCP\MCP\Undo_Store::read($token);
        if (null === $snapshot) {
            wp_send_json_error(['message' => esc_html__('That undo point was not found. It may have expired, or already been undone.', 'more-mcp')]);
        }
        ...
        if (empty($_POST['confirm'])) {
            ...
            wp_send_json_success([
                ...
                'message'  => esc_html__('This will restore the pre-operation state and consume the undo point. Confirm to proceed.', 'more-mcp'),
            ]);
        }
        ...
        try {
            $result = \More_MCP\MCP\Undo_Dispatcher::dispatch(['token' => $token]);
        } catch (\Throwable $e) {
            $this->log_admin_action('undo:run', ['op' => (string) ($snapshot['op'] ?? '')], ['status' => 'error']);
            wp_send_json_error(['message' => $e->getMessage()]);
        }

        
        
        $this->log_admin_action('undo:run', ['op' => (string) ($snapshot['op'] ?? '')], ['status' => 'success']);

        wp_send_json_success([
            'preview' => false,
            'result'  => is_array($result) ? $result : [],
            'message' => isset($result['message']) ? (string) $result['message'] : esc_html__('The operation was undone.', 'more-mcp'),
        ]);
    }
