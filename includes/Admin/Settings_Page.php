...
class Settings_Page {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_more_mcp_toggle_ability', [$this, 'ajax_toggle_ability']);
    ...
    public function ajax_toggle_ability() {
        ...
        $name    = isset($_POST['ability']) ? trim((string) wp_unslash($_POST['ability'])) : '';
        $enabled = isset($_POST['enabled']) && ('1' === (string) $_POST['enabled'] || 'true' === $_POST['enabled']);

        $importable = class_exists('\More_MCP\Abilities\Importer')
            ? array_flip(array_keys(\More_MCP\Abilities\Importer::importable_abilities()))
            : [];
        if ('' === $name || !isset($importable[$name])) {
            wp_send_json_error(['message' => esc_html__('Unknown ability.', 'more-mcp')]);
        }
    ...
    public function ajax_toggle_ability_namespace() {
        ...
        $importable = class_exists('\More_MCP\Abilities\Importer')
            ? array_keys(\More_MCP\Abilities\Importer::importable_abilities())
            : [];
        $ns_names = [];
        foreach ($importable as $ability_name) {
            $ability_name = (string) $ability_name;
            $slash        = strpos($ability_name, '/');
            if (false !== $slash && substr($ability_name, 0, $slash) === $namespace) {
                $ns_names[$ability_name] = true;
            }
        }
        if (empty($ns_names)) {
        ...
        $importable_flip = array_flip($importable);
        $settings = get_option('more_mcp_settings', []);
        $key      = \More_MCP\Abilities\Importer::ENABLED_KEY;
        $current  = isset($settings[$key]) && is_array($settings[$key]) ? $settings[$key] : [];
        ...
        foreach ($current as $n) {
            $n = is_string($n) ? trim($n) : '';
            if ('' !== $n && isset($importable_flip[$n]) && !isset($ns_names[$n])) {
                $set[$n] = true;
            }
        }
        if ($enabled) {
            foreach ($ns_names as $n => $_) {
                $set[$n] = true;
            }
        }
        $settings[$key] = array_values(array_keys($set));
        ...
        $this->log_admin_action(
            ...
            ['namespace' => $namespace, 'enabled' => $enabled, 'ability_count' => count($ns_names)],
            ['enabled_count' => count($settings[$key])]
        );
