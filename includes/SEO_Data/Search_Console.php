...
class Search_Console {
	...
	public static function execute_tool( string $name, array $args ) {
		...
		if ( ! Credentials::is_active( self::SLUG ) ) {
			throw new \Exception( 'Google Search Console is not connected. Add a service account key in Settings → External Services → SEO & analytics, then share the property with the service account email.' );
		}
		...
		switch ( $name ) {
			...
			case 'gsc_inspect_url':
				...
				return self::result( Http::request( self::SLUG, '/v1/urlInspection/index:inspect', array( 'method' => 'POST', 'body' => $body, 'timeout' => 30 ) ) );
		}
