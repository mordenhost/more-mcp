<?php
/**
 * Reads and writes SEO fields through whichever plugin is active.
 *
 * The contract every method here keeps: a write either lands in the location
 * the detected plugin actually reads, or it is refused by name. There is no
 * best-guess path. See Fields.php for why — GitHub #6 is what a plausible guess
 * looks like from the caller's side (success response, correct round-trip on
 * read, zero effect on the rendered page).
 *
 * Every write reads the value back out of storage and returns what it found, so
 * a sanitizer, a filter, or a plugin that normalises on save is visible rather
 * than reported as a clean success. This matches what wp_update_post already
 * does with saved_fields.
 *
 * @package More_MCP
 */
...
class Meta {
	...
	/**
	 * Read every supported field for an object.
	 *
	 * @param string $level   'post' or 'term'.
	 * @param int    $id      Post ID or term ID.
	 * @param string $context Taxonomy name, required at term level (Yoast keys
	 *                        its option by taxonomy, so we cannot look a term up
	 *                        without it).
	 * @return array Field name → value. Absent fields are returned as '' (or
	 *               false for noindex) rather than omitted, so a caller can tell
	 *               "supported but empty" from "not supported".
	 */
	...
	/**
	 * Write a set of fields. Returns what storage holds afterwards.
	 *
	 * @param array $fields Logical field name → value. Unsupported fields throw
	 *                      rather than being skipped: silently dropping a field
	 *                      the caller asked for is the failure this subsystem
	 *                      exists to prevent.
	 * @throws \Exception When a field is not supported by the active plugin at
	 *                    this level.
	 */
	public static function write( string $level, int $id, string $context, array $fields ): array {
		...
		// Validate the whole set before writing any of it. A partial write
		// leaves the caller unable to tell which fields landed.
		...
		foreach ( $fields as $field => $value ) {
			...
			// Read back rather than echoing the input.
	...
	/**
	 * Refuse an unsupported field with a message that says what to do instead.
	 *
	 * @throws \Exception Always, when unsupported.
	 */
	private static function assert_supported( string $plugin, string $level, string $field ): void {
		...
		// Known field, unsupported by this plugin at this level. Name the
		// plugins where it does work — a caller can then decide rather than
		// retrying blindly.
		$elsewhere = array();
	...
	/**
	 * Turn plugin slugs into a readable list for an error message.
	 */
	...
	/**
	 * Read one field, dispatching on the plugin's storage strategy.
	 *
	 * @return string|bool String for text fields, bool for noindex.
	 */
	private static function read_field( string $plugin, string $level, int $id, string $context, string $field ) {
		$spec = Fields::spec( $plugin, $level );
		$map  = $spec[ $field ];

		// noindex carries its own strategy because no two plugins encode it alike.
		if ( 'noindex' === $field ) {
	...
	/**
	 * Read noindex. Returns a bool for every plugin, whatever the encoding.
	 *
	 * Note the tri-state cases: Yoast term-level and TSF both distinguish
	 * "inherit the default" from an explicit index. Both collapse to false here —
	 * the field is documented as "is this URL explicitly noindexed" — and the
	 * raw stored value is exposed separately by the read tool so the distinction
	 * is not lost.
	 */
	...
	/**
	 * Write one field, dispatching on strategy.
	 */
	private static function write_field( string $plugin, string $level, int $id, string $context, string $field, $value ): void {
		...
		// canonical is a URL, not free text: esc_url_raw preserves the scheme,
		// host, and query string that sanitize_text_field would strip or mangle.
		...
		switch ( $spec['strategy'] ) {
			...
			case 'meta_array':
				// Read-modify-write: the array holds every other field, so a
				// blind overwrite would discard them.
				$bag         = get_post_meta( $id, $spec['key'], true );
				$bag         = is_array( $bag ) ? $bag : array();
				$bag[ $map ] = $value;
	...
	/**
	 * Write noindex in whatever encoding the plugin uses.
	 */
	private static function write_noindex( array $spec, array $map, string $level, int $id, string $context, bool $noindex ): void {
		switch ( $map['strategy'] ) {
			...
			case 'robots_array':
				// Preserve every other robots directive (nofollow, noarchive,
				// noimageindex…) and replace only the index/noindex member.
				$current = 'term' === $level
					? get_term_meta( $id, $map['key'], true )
					: get_post_meta( $id, $map['key'], true );
			...
			case 'aioseo_robots':
				// robots_default must be cleared, or AIOSEO ignores the explicit
				// flag and falls back to the global setting.
	...
	// ==================== Yoast taxonomy option ====================

	/**
	 * One term's slice of wpseo_taxonomy_meta.
	 *
	 * Shape: [ taxonomy => [ term_id => [ field => value ] ] ]. Yoast stores
	 * term_id as an int key, but WordPress serialization and JSON round-trips
	 * can turn it into a string, so both are checked.
	 */
	...
	/**
	 * Merge fields into one term's slice of wpseo_taxonomy_meta.
	 *
	 * Read-modify-write is mandatory here, not an optimisation: this single
	 * option holds the SEO settings for every term in every taxonomy on the
	 * site. Overwriting it wholesale to set one term's noindex would wipe the
	 * rest. GitHub #6 raised exactly this concern as the reason read access is a
	 * prerequisite for writing.
	 *
	 * @throws \Exception When the option cannot be written.
	 */
	private static function yoast_term_write( string $taxonomy, int $term_id, array $fields ): void {
		...
		// Yoast writes int keys; normalise so we update rather than duplicate a
		// row that arrived as a string key.
		$key = $term_id;
		...
		if ( false === $result ) {
			...
			// not an error. Distinguish by reading back.
	...
	// ==================== AIOSEO custom tables ====================

	/**
	 * One row from an AIOSEO table, as an associative array, or null.
	 */
	private static function aioseo_row( array $spec, int $id ): ?array {
		...
		$row   = $wpdb->get_row(
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- table name comes from the Fields catalogue, never from input.
	...
	/**
	 * Upsert columns on an AIOSEO row.
	 *
	 * AIOSEO 4.x will not render values that are not in its own table, so this
	 * has to insert when no row exists yet — a post with no AIOSEO row is the
	 * normal state until someone opens its SEO panel.
	 *
	 * @throws \Exception When the table is missing (AIOSEO active but not
	 *                    migrated) or the write fails.
	 */
	private static function aioseo_write( array $spec, int $id, array $columns, string $context ): void {
		...
		// esc_like the table name: it contains underscores ($wpdb->prefix and the
		// aioseo_ table names), and `_` is a single-character wildcard in LIKE. An
		// unescaped underscore would match a differently-named table on prefixes
		// that use one (e.g. wpXaioseo_posts), so a false positive is possible.
		...
		if ( null === $row ) {
			...
			// aioseo_terms additionally requires the taxonomy.
	...
	/**
	 * Raw stored values for a level, for diagnostics.
	 *
	 * The read tool exposes this alongside the normalised fields because the
	 * normalisation is lossy in one specific way: tri-state noindex ('default'
	 * on Yoast terms, 0 on TSF) collapses to false. Someone auditing why an
	 * archive is indexable needs to know whether it says "inherit" or "index".
	 */
