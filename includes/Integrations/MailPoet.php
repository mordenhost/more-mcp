...
class MailPoet {
	...
	public static function get_tools() {
		...
		return array(
			...
			array(
				'name'        => 'mailpoet_get_segments',
				'description' => 'Read the MailPoet segment/list catalogue: for each segment its id, name, type (default, dynamic, wp_users, woocommerce_users, and similar), and description. Soft-deleted segments are excluded. Returns segment definitions only — never a subscriber record, email address, segment membership, or any personal data. Read-only diagnostic; cannot create, edit, or delete a segment.',
				'inputSchema' => array(
					'type'       => 'object',
					'properties' => new \stdClass(),
				),
	...
	private static function get_segments() {
		...
		if ( ! self::table_exists( $table ) ) {
			return array(
				'provider'  => 'mailpoet',
				'available' => false,
				'message'   => 'MailPoet segments table was not found; cannot read segments on this version.',
			);
		}

		
		
		$rows = $wpdb->get_results( "SELECT id, name, type, description FROM {$table} WHERE deleted_at IS NULL ORDER BY id", ARRAY_A );
