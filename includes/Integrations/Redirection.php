...
class Redirection {
	...
	public static function get_manifest() {
		return array(
			'providers'    => array( 'redirection' ),
			'capabilities' => array( 'redirects' ),
			'kind'         => 'plugin',
		);
	}
	...
	public static function get_tools() {
		...
		return array(
			...
			array(
				'name'        => 'redirection_get_logs_aggregate',
				'description' => 'Aggregate Redirection log counts grouped by ip, url, or agent (the only grouping keys the plugin\'s get_grouped accepts). Returns per-key hit counts, never individual log rows — the log records client IPs and user agents, so only aggregates are surfaced. Read-only against redirection_logs.',
				'inputSchema' => array(
					'type'       => 'object',
					'properties' => array(
						'group'    => array( 'type' => 'string', 'enum' => array( 'ip', 'url', 'agent' ), 'description' => 'Grouping key. Default url.' ),
						'per_page' => array( 'type' => 'integer', 'description' => 'How many groups to return (5-200, default 25).' ),
						'page'     => array( 'type' => 'integer', 'description' => 'Zero-based page offset. Default 0.' ),
					),
				),
			),
			array(
				'name'        => 'redirection_get_groups',
				'description' => 'List Redirection groups with verbatim status (enabled/disabled), position, and module id. Raw table read, so the status enum and position are preserved exactly as stored. Read-only.',
				'inputSchema' => array(
					'type'       => 'object',
					'properties' => new \stdClass(),
				),
	...
	public static function execute_tool( $name, $args ) {
		...
		switch ( $name ) {
			...
			case 'redirection_get_logs_aggregate':
				return self::get_logs_aggregate( $args );
			case 'redirection_get_groups':
	...
	private static function get_logs_aggregate( $args ) {
		$group = isset( $args['group'] ) ? sanitize_key( $args['group'] ) : 'url';
		if ( ! in_array( $group, array( 'ip', 'url', 'agent' ), true ) ) {
			throw new \Exception( 'group must be one of: ip, url, agent.' );
		}
		...
		$class = class_exists( 'Redirection\\Log\\Redirect' ) ? 'Redirection\\Log\\Redirect' : ( class_exists( 'Red_Redirect_Log' ) ? 'Red_Redirect_Log' : null );
		if ( null === $class || ! method_exists( $class, 'get_grouped' ) ) {
			throw new \Exception( 'Redirection log aggregation is not available in this version.' );
		}

		$result = $class::get_grouped( $group, array(
			'per_page' => $per_page,
			'page'     => $page,
		) );
	...
	private static function get_groups() {
		global $wpdb;
		$table = $wpdb->prefix . 'redirection_groups';
		$rows  = $wpdb->get_results( "SELECT id, name, status, module_id, position FROM {$table} ORDER BY position, id", ARRAY_A );
		if ( ! is_array( $rows ) ) {
			return array( 'provider' => 'redirection', 'groups' => array() );
		}
		...
		return array( 'provider' => 'redirection', 'groups' => $groups );
	}
}
