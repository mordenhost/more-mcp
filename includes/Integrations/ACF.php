	...
	public static function execute_tool( $name, $args ) {
		...
		switch ( $name ) {
			...
			case 'acf_update_field':
				...
				$target = get_field_object( $field_name, $post_id, false, false );
				if ( ! is_array( $target ) || empty( $target['key'] ) ) {
					throw new \Exception(
						'No ACF field "' . esc_html( $field_name ) . '" is registered for post '
						. esc_html( (string) $post_id ) . '. Nothing was written. The field name or key '
						. 'must resolve to a field whose field group is attached to this post — check that '
						. 'the field group exists and its location rules include this post. (A field group '
						. 'created via SCF with nested fields that did not persist will not resolve here — see GitHub #131.)'
					);
				}
