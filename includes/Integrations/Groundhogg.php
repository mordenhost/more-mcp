...
class Groundhogg {
	...
	public static function get_tools() {
		...
		return array(
			...
			array(
				'name'        => 'groundhogg_get_tags',
				'description' => 'Read the Groundhogg tag catalogue: for each tag its id, name, and slug. Returns tag definitions only — never a contact record, email address, or tag membership. Read-only diagnostic; cannot create, edit, or delete a tag.',
				'inputSchema' => array(
					'type'       => 'object',
					'properties' => new \stdClass(),
				),
	...
	private static function get_tags() {
		...
		if ( ! self::table_exists( $table ) ) {
			return array(
				'provider'  => 'groundhogg',
				'available' => false,
				'message'   => 'Groundhogg tags table was not found; cannot read tags on this version.',
			);
		}

		
		
		$rows = $wpdb->get_results( "SELECT tag_id, tag_name, tag_slug FROM {$table} ORDER BY tag_id", ARRAY_A );
		...
		if ( is_array( $rows ) ) {
			foreach ( $rows as $row ) {
				$tags[] = array(
					...
					'tag_slug' => isset( $row['tag_slug'] ) ? (string) $row['tag_slug'] : '',
				);
			}
		}
