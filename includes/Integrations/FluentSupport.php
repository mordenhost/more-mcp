...
class FluentSupport {
	...
	public static function get_tools() {
		...
		return array(
			...
			array(
				'name'        => 'fluentsupport_get_queue',
				'description' => 'Read Fluent Support queue depth: how many tickets sit unassigned, how many wait for an agent reply, and how long the waiting ones have waited (average and longest wait). Aggregate scalars only, from the desk\'s own queue accessors. Never a ticket, a title or number, content, or any customer or agent identity. Read-only diagnostic.',
				'inputSchema' => array(
					'type'       => 'object',
					'properties' => new \stdClass(),
				),
	...
	public static function execute_tool( $name, $args ) {
		...
		if ( 'fluentsupport_get_queue' === $name ) {
	...
	private static function get_queue() {
		$unassigned = null;
		$helper = '\FluentSupport\App\Services\TicketHelper';
		if ( class_exists( $helper ) && method_exists( $helper, 'countUnassignedTickets' ) ) {
			try {
				$unassigned = (int) call_user_func( array( $helper, 'countUnassignedTickets' ) );
			} catch ( \Throwable $e ) {
				$unassigned = null;
			}
		}
		...
		$awaiting = null;
		$stat = '\FluentSupport\App\Modules\StatModule';
		if ( class_exists( $stat ) && method_exists( $stat, 'countAwaitingTickets' ) ) {
			try {
				$awaiting = (int) call_user_func( array( $stat, 'countAwaitingTickets' ) );
			} catch ( \Throwable $e ) {
				$awaiting = null;
			}
		}
		...
		$wait = null;
		$reporting = '\FluentSupport\App\Modules\Reporting\Reporting';
		if ( class_exists( $reporting ) && method_exists( $reporting, 'getActiveStats' ) ) {
			try {
				$report = new $reporting();
				$active = $report->getActiveStats();
				if ( is_array( $active ) ) {
					$wait = array(
						'waiting_tickets' => isset( $active['waiting_tickets'] ) ? (int) $active['waiting_tickets'] : null,
						'average_wait'    => isset( $active['average_waiting'] ) && is_scalar( $active['average_waiting'] ) ? (string) $active['average_waiting'] : null,
						'longest_wait'    => isset( $active['max_waiting'] ) && is_scalar( $active['max_waiting'] ) ? (string) $active['max_waiting'] : null,
					);
				}
			} catch ( \Throwable $e ) {
				$wait = null;
		...
		if ( null === $unassigned && null === $awaiting && null === $wait ) {
			return array(
				'provider'  => 'fluentsupport',
				'available' => false,
				'message'   => 'Fluent Support queue accessors were not found; cannot read queue depth on this version.',
			);
		}
