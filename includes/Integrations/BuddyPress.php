...
class BuddyPress {
	...
	public static function get_tools() {
		...
		return array(
			...
			array(
				'name'        => 'bp_get_activity_stats',
				'description' => 'Read BuddyPress activity-stream scale: the total number of recorded activity items plus a per-component breakdown (which component each item belongs to: activity/status updates, groups, blogs, profiles, friends, messages, and any other registered component), counted through BuddyPress\'s own activity query in count-only mode. Aggregate counts only, never an activity item, its content, or an author identity. Read-only diagnostic.',
				'inputSchema' => array(
					'type'       => 'object',
					'properties' => new \stdClass(),
				),
	...
	private static function get_activity_stats() {
		if ( ! function_exists( 'bp_activity_get' ) || ! function_exists( 'bp_is_active' ) ) {
			return array(
				'provider'  => 'buddypress',
				'available' => false,
				'message'   => 'The BuddyPress activity query accessor was not found.',
			);
		}

		$components = array( 'activity' );
		if ( function_exists( 'buddypress' ) && is_object( buddypress() ) && isset( buddypress()->activity->id ) ) {
			$components = array( buddypress()->activity->id );
			foreach ( self::COMPONENT_KEYS as $component => $field ) {
				if ( bp_is_active( $component ) && ! in_array( $component, $components, true ) ) {
			...
			try {
				$result = bp_activity_get(
					array(
						...
						'show_hidden'      => true,
						'spam'             => 'all',
						'filter'           => array( 'object' => $component ),
					)
				);
			} catch ( \Throwable $e ) {
				continue;
			}
			$n = ( is_array( $result ) && isset( $result['total'] ) ) ? (int) $result['total'] : 0;
			$counts[ sanitize_key( $component ) ] = $n;
			$total                               += $n;
		}
