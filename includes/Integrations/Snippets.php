...
class Snippets {
	...
	public static function undo_write( array $snapshot ) {
		$op = isset( $snapshot['op'] ) ? (string) $snapshot['op'] : '';
		$id = isset( $snapshot['snippet_id'] ) ? (int) $snapshot['snippet_id'] : 0;
		if ( $id <= 0 ) {
			throw new \Exception( 'Undo snapshot has no snippet id.' );
		}
		if ( 'snippet_create' === $op ) {
			return self::undo_create( $id, $snapshot );
		}
		if ( 'snippet_update' === $op ) {
			return self::undo_update( $id, $snapshot );
		}
		throw new \Exception( esc_html( 'Unknown snippets undo op: ' . $op ) );
	}

	private static function undo_create( $id, $snapshot ) {

		
		if ( ! function_exists( 'delete_snippet' ) ) {
			throw new \Exception( esc_html( sprintf(
				'The Code Snippets delete API (delete_snippet) is unavailable. Read API (get_snippets) present: %s. Plugin version: %s. See the write-API note: the plugin is active but its ops functions did not load in this request.',
				function_exists( 'get_snippets' ) ? 'yes' : 'no',
				defined( 'CODE_SNIPPETS_VERSION' ) ? CODE_SNIPPETS_VERSION : 'unknown'
			) ) );
		}
		$s = \get_snippet( $id );
		if ( ! is_object( $s ) || (int) ( $s->id ?? 0 ) !== $id ) {
			...
			return array(
				...
				'message'        => 'The snippet had already been deleted before this undo ran. Nothing was removed.',
			);
		}
		$name   = (string) ( $s->name ?? '' );
		$active = (bool) ( $s->active ?? false );
		if ( ! empty( $s->locked ) ) {
			...
			throw new \Exception( esc_html( sprintf(
				'Snippet %d is locked in Code Snippets, which refuses to delete it. Unlock it there and delete it manually if you still want it gone.',
				$id
			) ) );
		}
		if ( ! \delete_snippet( $id ) ) {
			throw new \Exception( esc_html( sprintf( 'Code Snippets declined to delete snippet %d; treat this undo as not performed.', $id ) ) );
		}

		$after = \get_snippet( $id );
		$gone  = ! is_object( $after ) || (int) ( $after->id ?? 0 ) !== $id;
		if ( ! $gone ) {
			throw new \Exception( esc_html( sprintf( 'Snippet %d still exists after the delete; treat this undo as not performed.', $id ) ) );
		}
		return array(
			'success'            => true,
			'op'                 => 'snippet_create',
			'id'                 => $id,
			'removed_name'       => $name,
			'active_at_deletion' => $active,
			'summary'            => (string) ( $snapshot['summary'] ?? '' ),
			'message'            => $active
				? 'Snippet removed. It was ACTIVE at deletion time: it had been activated after it was created, so this undo removed running code.'
				: 'Snippet removed. It was disabled at deletion time.',
		);
	}

	private static function undo_update( $id, $snapshot ) {

		self::require_write_api();
		if ( ! array_key_exists( 'prev_code', $snapshot ) && ! array_key_exists( 'prev_name', $snapshot ) ) {
			throw new \Exception( 'Undo snapshot has no previous code or name to restore.' );
		}
		$s = \get_snippet( $id );
		if ( ! is_object( $s ) || (int) ( $s->id ?? 0 ) !== $id ) {
			throw new \Exception( esc_html( sprintf(
				'Snippet %d no longer exists, so its previous code and name cannot be restored.',
				$id
			) ) );
		}
		if ( ! empty( $s->locked ) ) {
			throw new \Exception( esc_html( sprintf(
				'Snippet %d is locked in Code Snippets, which silently discards changes to its code and name, so this undo could not land. Unlock it there, then redeem this undo again.',
				$id
			) ) );
		}
		$type = (string) ( $s->type ?? '' );
		if ( ! in_array( $type, array( 'css', 'js' ), true ) ) {
			throw new \Exception( esc_html( sprintf(
				'Snippet %d is now type "%s"; restoring code into it through the undo path is refused, exactly as snippet_update refuses it. Resolve its scope in the Code Snippets UI first.',
				$id,
				'' !== $type ? $type : 'unknown'
			) ) );
		}

		$prev_code  = (string) ( $snapshot['prev_code'] ?? '' );
		$prev_name  = (string) ( $snapshot['prev_name'] ?? '' );
		$was_active = (bool) ( $s->active ?? false );

		
		
		if ( array_key_exists( 'prev_code', $snapshot ) ) {
			$s->code = $prev_code;
		}
		if ( array_key_exists( 'prev_name', $snapshot ) ) {
			$s->name = $prev_name;
		}
		
		$restored = \save_snippet( $s );
		if ( ! is_object( $restored ) ) {
			throw new \Exception( 'Failed to restore the snippet.' );
		}

		
		
		$discarded = array();
		if ( array_key_exists( 'prev_code', $snapshot ) && (string) ( $restored->code ?? '' ) !== $prev_code ) {
			$discarded[] = 'code';
		}
		if ( array_key_exists( 'prev_name', $snapshot ) && (string) ( $restored->name ?? '' ) !== $prev_name ) {
			$discarded[] = 'name';
		}
		if ( ! empty( $discarded ) ) {
			throw new \Exception( esc_html( sprintf(
				'Snippet %d was not restored as sent: Code Snippets kept its own value for %s. Treat this undo as not performed and check the snippet in the Code Snippets admin.',
				$id,
				implode( ' and ', $discarded )
			) ) );
		}
		...
		return array(
			...
			'id'                    => $id,
			'summary'               => (string) ( $snapshot['summary'] ?? '' ),
			'active_left_unchanged' => true,
			'active_at_restore'     => $was_active,
			'message'               => 'Snippet restored to its pre-write state. Its active state was left untouched'
				. ( $was_active ? ' — it was active and still is.' : '.' ),
		);
	}
