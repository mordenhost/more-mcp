...
class Snippets {
	...
	private static function update( $args ) {
		...
		if ( ! function_exists( '\save_snippet' ) ) {
			throw new \Exception( 'The Code Snippets plugin API is unavailable.' );
		}
		if ( null !== $new_name ) {
			$s->name = $new_name;
		}
	...
	private static function save_new( $name, $desc, $code, $scope ) {
		if ( ! function_exists( '\get_snippet' ) || ! function_exists( '\save_snippet' ) ) {
			throw new \Exception( 'The Code Snippets plugin API is unavailable.' );
		}
