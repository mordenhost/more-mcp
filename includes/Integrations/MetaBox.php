...
class MetaBox {
	...
	private static function update_field( $post_id, $field_id, $value ) {
		...
		$stored = rwmb_meta( $field_id, array(), $post_id );
		$state  = Write_Verify::state_of( $stored, $value, $prior );

		Write_Verify::refuse_if_discarded(
			$state,
			__( 'field value', 'more-mcp' ),
			__( 'Check whether the field is registered for this post type and whether its type accepts this value shape.', 'more-mcp' )
		);

		$verify = Write_Verify::report(
			$state,
			__( 'field value', 'more-mcp' ),
			__( 'Meta Box field types transform some values on save, so a rewritten value is expected for some types.', 'more-mcp' )
		);
		...
		return array_merge(
			array(
				...
				'value'    => self::flatten_value( $stored ),
			),
			$verify,
			array( 'undo' => $undo )
		);
