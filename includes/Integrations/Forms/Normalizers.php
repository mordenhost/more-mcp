...
class Normalizers {
	...
	public static function verify_write( $stored, $expected, $property, $prior = Write_Verify::NO_PRIOR ) {
		$subject = sprintf(
			/* translators: %s: entry property name, e.g. "status" or "is_read". */
			__( 'entry %s', 'more-mcp' ),
			(string) $property
		);
		$state = Write_Verify::state_of( $stored, $expected, $prior );

		
		Write_Verify::refuse_if_discarded( $state, $subject );

		return Write_Verify::report( $state, $subject );
	}
}
