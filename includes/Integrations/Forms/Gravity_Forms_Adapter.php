...
class Gravity_Forms_Adapter {
	...
	public static function gf_update_entry_status( $entry_id, $status ) {
		...
		if ( ! is_array( $fresh ) || ! isset( $fresh[ $property ] ) ) {
			$stored = Write_Verify::UNREADABLE;
		} elseif ( 'is_read' === $property ) {
			$stored = empty( $fresh[ $property ] ) ? '0' : '1';
		} else {
			$stored = $fresh[ $property ];
		}
		$verify = Normalizers::verify_write( $stored, $value, $property, $prior['value'] );
	...
	public static function gf_trash_entry( $entry_id ) {
		...
		$verify = Normalizers::verify_write(
			( is_array( $fresh ) && isset( $fresh['status'] ) ) ? $fresh['status'] : Write_Verify::UNREADABLE,
			'trash',
			'status',
			$prior['value']
		);
