...
class WPForms_Adapter {
	...
	public static function wpf_update_entry_status( $entry_id, $status ) {
		...
		$fresh    = $handler->get( (int) $entry_id );
		if ( ! is_object( $fresh ) || ! isset( $fresh->{$property} ) ) {
			$stored = Write_Verify::UNREADABLE;
		} elseif ( 'viewed' === $property ) {
			$stored = empty( $fresh->{$property} ) ? 0 : 1;
		} else {
			$stored = $fresh->{$property};
		}
		$verify = Normalizers::verify_write( $stored, $data[ $property ], $property, $prior['value'] );
	...
	public static function wpf_trash_entry( $entry_id ) {
		...
		$verify = Normalizers::verify_write(
			( is_object( $fresh ) && isset( $fresh->status ) ) ? $fresh->status : Write_Verify::UNREADABLE,
			'trash',
			'status',
			$prior['value']
		);
