...
class Runtime {
	...
	private static $widgets_registration_attempted = false;
	...
	public static function execute_tool( $name, $args ) {
		...
		switch ( $name ) {
			...
			case 'elementor_list_widget_types':
				return self::list_widget_types( $args );

			case 'elementor_get_widget_type_schema':
				return self::get_widget_type_schema( $args );
			...
			case 'elementor_set_template_conditions':
				return self::set_template_conditions( $args );

			
			case 'elementor_list_fonts':
				return Assets::list_fonts();
			case 'elementor_get_font':
				return Assets::get_font( $args );
			case 'elementor_create_font':
				return Assets::create_font( $args );
			case 'elementor_update_font':
				return Assets::update_font( $args );
			case 'elementor_delete_font':
				return Assets::delete_font( $args );
			case 'elementor_list_icon_sets':
				return Assets::list_icon_sets();
			case 'elementor_get_icon_set':
				return Assets::get_icon_set( $args );
			case 'elementor_delete_icon_set':
				return Assets::delete_icon_set( $args );

			
			case 'elementor_list_code':
				return Code::list_code();
			case 'elementor_get_code':
				return Code::get_code( $args );
			case 'elementor_create_code':
				return Code::create_code( $args );
			case 'elementor_update_code':
				return Code::update_code( $args );
			case 'elementor_delete_code':
				return Code::delete_code( $args );
	...
	private static function update_widget( $args ) {
		...
		$pre_op_tree = wp_json_encode( $tree );

		
		$rev = self::snapshot_revision( $post_id );
		...
		self::record_edit_trail( $post_id, $element_id, 'update_widget', array_keys( (array) $args['settings'] ) );
		...
		$verify_tree = self::load_tree_or_null( $post_id );
		$verify_el   = ( null === $verify_tree ) ? null : self::find_element_by_id( $verify_tree, $element_id );
		if ( null === $verify_tree ) {
			$stored = \More_MCP\Integrations\Write_Verify::UNREADABLE;
		} else {
			$stored = ( $verify_el !== null && isset( $verify_el['settings'] ) && is_array( $verify_el['settings'] ) )
				? $verify_el['settings']
				: [];
		}
		$state = \More_MCP\Integrations\Write_Verify::state_of( $stored, $new_settings, $existing );
		...
		$undo = \More_MCP\MCP\Undo_Store::store( [
			...
			'pre_op_state' => [ 'elementor_data' => $pre_op_tree ],
		] );

		return self::with_invalidation( self::with_revision( array_merge( [
			'success'      => true,
			'written'      => true,
			'post_id'      => $post_id,
			'element_id'   => $element_id,
			...
			'widget_type'  => $widget_type !== '' ? $widget_type : null,
			'mode'         => $replace ? 'replace' : 'merge',
			'settings'     => ( \More_MCP\Integrations\Write_Verify::UNREADABLE === $stored ) ? [] : $stored,
			'keys_removed' => $removed_keys,
		],
			\More_MCP\Integrations\Write_Verify::report( $state, __( 'element settings', 'more-mcp' ) ),
			[
				'undo'     => $undo,
	...
	private static function delete_widget( $args ) {
		...
		self::prune_edit_trail( $post_id, $element_id );
		...
		$verify_tree = self::load_tree_or_null( $post_id );
		if ( null === $verify_tree ) {
			$state       = \More_MCP\Integrations\Write_Verify::UNKNOWN;
			$still_there = null;
		} else {
			$still_there = self::find_element_by_id( $verify_tree, $element_id ) !== null;
			$state       = \More_MCP\Integrations\Write_Verify::state_of_presence( ! $still_there );
		}
		...
		return self::with_invalidation( self::with_revision( array_merge( [
			...
			\More_MCP\Integrations\Write_Verify::report( $state, __( 'element removal', 'more-mcp' ) ),
			[
				'undo'     => $undo,
				'edit_url' => admin_url( 'post.php?post=' . $post_id . '&action=elementor' ),
			]
	...
	private static function move_widget( $args ) {
		...
		self::record_edit_trail( $post_id, $element_id, 'move_widget', [] );
		...
		if ( null === $verify_tree ) {
			...
			$state = \More_MCP\Integrations\Write_Verify::state_of_presence(
				self::find_element_by_id( $verify_tree, $element_id ) !== null
		...
		return self::with_invalidation( self::with_revision( array_merge( [
			...
			\More_MCP\Integrations\Write_Verify::report( $state, __( 'element move', 'more-mcp' ) ),
			[
				'undo'     => $undo,
				'edit_url' => admin_url( 'post.php?post=' . $post_id . '&action=elementor' ),
			]
	...
	private static function theme_builder_widgets() {
		return [
			'theme-post-title'          => [ 'binding' => 'title',   'requires_binding' => false, 'tag' => 'post-title' ],
			'theme-page-title'          => [ 'binding' => 'title',   'requires_binding' => false, 'tag' => 'page-title' ],
			'theme-archive-title'       => [ 'binding' => 'title',   'requires_binding' => false, 'tag' => 'archive-title' ],
			'theme-site-title'          => [ 'binding' => 'title',   'requires_binding' => false, 'tag' => 'site-title' ],
			'theme-post-featured-image' => [ 'binding' => 'image',   'requires_binding' => false, 'tag' => 'post-featured-image' ],
			'theme-site-logo'           => [ 'binding' => 'image',   'requires_binding' => false, 'tag' => 'site-logo' ],
			'theme-post-excerpt'        => [ 'binding' => 'excerpt', 'requires_binding' => false, 'tag' => 'post-excerpt' ],
			'theme-post-content'        => [ 'binding' => null,      'requires_binding' => false, 'tag' => null ],
		];
	}

	
	private static function ensure_widgets_registered() {
		if ( self::$widgets_registration_attempted ) {
			return;
		}
		self::$widgets_registration_attempted = true;
		...
		if ( function_exists( 'did_action' ) && function_exists( 'do_action' ) ) {
			if ( 0 === did_action( 'elementor/init' ) ) {
				try {
	...
	private static function validate_dynamic_bindings( array $element ) {
		...
		if ( ! is_string( $value ) || '' === trim( $value ) ) {
			$errors[] = sprintf(
				'Widget "%s" (id %s) has a __dynamic__["%s"] binding that is not a non-empty string. '
				. 'Either omit it (the widget auto-binds to its "%s" dynamic tag via the control default and renders live data) '
				. 'or set it to a dynamic-tag shortcode: "[elementor-tag id=\"\" name=\"%s\" settings=\"%%7B%%7D\"]".',
				$widget_type, $id, $key, $tag, $tag
			);
			return $errors;
		}

		
		if ( ! preg_match( '/\[elementor-tag\b([^\]]*)\]/', $value, $m ) ) {
			$errors[] = sprintf(
				'Widget "%s" (id %s) __dynamic__["%s"] is not an Elementor dynamic-tag shortcode. '
				. 'Expected "[elementor-tag id=\"\" name=\"%s\" settings=\"%%7B%%7D\"]", or omit it to let the widget auto-bind to its "%s" tag.',
				$widget_type, $id, $key, $tag, $tag
			);
			return $errors;
		}
		...
		if ( ! preg_match( '/\bname="([^"]*)"/', $m[1], $nm ) ) {
			$errors[] = sprintf(
				'Widget "%s" (id %s) __dynamic__["%s"] is missing a name= on its elementor-tag shortcode. '
				. 'It must name this widget\'s dynamic tag: name="%s".',
				$widget_type, $id, $key, $tag
			);
		} elseif ( $nm[1] !== $tag ) {
			$errors[] = sprintf(
				'Widget "%s" (id %s) __dynamic__["%s"] binds dynamic tag name="%s", but this widget\'s tag is "%s". '
				. 'Use name="%s", or omit the binding to auto-bind to it.',
				$widget_type, $id, $key, $nm[1], $tag, $tag
			);
		}
		return $errors;
	}
	...
	private static function unregistered_widget_message( $widget_type, $provider ) {
		...
		$hint = ' Use a curated type (' . implode( ', ', self::$curated_widget_types ) . '), an Elementor V4 atomic widget (a-* / e-*), or any slug returned by Elementor\'s widget registry.';
		if ( in_array( $provider, [ 'official_pro', 'pro_elements', 'official_pro_plus_pro_elements' ], true ) ) {
			$hint = ' Elementor Pro is active here, so this is not a missing-Pro problem: the slug is not a registered widget on this site (check for a typo, or a widget supplied by an addon that is not installed).' . $hint;
		}
		return $base . $hint;
	}
	...
	private static function load_tree_or_null( $post_id ) {
		$raw = get_post_meta( $post_id, '_elementor_data', true );
		if ( empty( $raw ) ) {
			return null;
		}
	...
	private static function snapshot_revision( $post_id ) {
		$post_id = (int) $post_id;
		...
		if ( ! function_exists( 'wp_save_post_revision' ) || ! function_exists( 'add_filter' ) || ! function_exists( 'remove_filter' ) ) {
			return [
				'created' => false,
				'reason'  => 'WordPress revision functions are not available in this context, so no restore point was created.',
			];
		}
		...
		if ( ! $post ) {
			return [
				'created' => false,
				'reason'  => 'The post could not be loaded, so no restore point was created.',
			];
		}
		...
		if ( function_exists( 'wp_revisions_enabled' ) && ! wp_revisions_enabled( $post ) ) {
			return [
				'created' => false,
				'reason'  => 'Revisions are disabled for this post, either because WP_POST_REVISIONS is off or because the post type does not support revisions. No restore point was created; the undo token is the only rollback for this write.',
			];
		}

		$force_revision = static function () {
			return false;
		};
		...
		if ( is_wp_error( $revision_id ) ) {
			return [
				'created' => false,
				'reason'  => 'WordPress could not create the revision: ' . $revision_id->get_error_message(),
			];
		}
		...
		if ( empty( $revision_id ) ) {
			return [
				'created' => false,
				'reason'  => 'WordPress created no revision for this post. No restore point exists for this write beyond the undo token.',
			];
		}
	...
	private static function with_revision( array $response, array $rev ) {
		if ( ! empty( $rev['created'] ) ) {
			$response['revision'] = [
				'created'     => true,
				'revision_id' => (int) $rev['revision_id'],
				'restore_with' => 'wp_restore_revision',
			];
		} else {
			$response['revision'] = [
				'created' => false,
				'reason'  => (string) ( $rev['reason'] ?? 'No restore point was created.' ),
			];
		}
		return $response;
	}
	...
	private static function record_edit_trail( $post_id, $element_id, $tool, array $keys_touched = [] ) {
		if ( class_exists( '\More_MCP\MCP\Edit_Trail_Store' ) ) {
			\More_MCP\MCP\Edit_Trail_Store::record( $post_id, $element_id, $tool, $keys_touched );
		}
	}

	
	private static function prune_edit_trail( $post_id, $element_id ) {
		if ( class_exists( '\More_MCP\MCP\Edit_Trail_Store' ) ) {
			\More_MCP\MCP\Edit_Trail_Store::delete_element( $post_id, $element_id );
		}
	}

	
	private static function get_edit_trail( $post_id ) {
		if ( class_exists( '\More_MCP\MCP\Edit_Trail_Store' ) ) {
			return \More_MCP\MCP\Edit_Trail_Store::get_for_post( $post_id );
		}
		return [];
	}

	
	private static function format_trail_entry( array $entry ) {
		$at = isset( $entry['at'] ) ? (int) $entry['at'] : 0;
		$out = [
			'tool' => (string) ( $entry['tool'] ?? '' ),
			'keys' => isset( $entry['keys'] ) && is_array( $entry['keys'] ) ? array_values( $entry['keys'] ) : [],
			'at'   => $at,
		];
		if ( $at > 0 && function_exists( 'wp_date' ) ) {
			$out['at_formatted'] = wp_date( 'Y-m-d H:i', $at );
		}
		return $out;
	}
	...
	private static function replace_text( $args ) {
		...
		return self::with_invalidation( self::with_revision( [
			'success'       => true,
			'post_id'       => $post_id,
	...
	private static function get_page_outline( $args ) {
		...
		$outline = self::build_outline( $tree, 0, self::get_edit_trail( $post_id ) );
		$post = get_post( $post_id );
	...
	private static function get_widget_settings( $args ) {
		...
		$trail = self::get_edit_trail( $post_id );
		if ( isset( $trail[ $element_id ] ) && is_array( $trail[ $element_id ] ) ) {
			$result['mcp_edits'] = self::format_trail_entry( $trail[ $element_id ] );
		}
	...
	private static function build_outline( $elements, $depth, array $trail = [] ) {
		$out = [];
		if ( $depth > 6 ) {
		...
		foreach ( $elements as $el ) {
			...
			if ( '' !== $id && isset( $trail[ $id ] ) && is_array( $trail[ $id ] ) ) {
				$entry['mcp_edits'] = self::format_trail_entry( $trail[ $id ] );
			}
			if ( isset( $el['elements'] ) && is_array( $el['elements'] ) && count( $el['elements'] ) > 0 ) {
				$entry['children'] = self::build_outline( $el['elements'], $depth + 1, $trail );
			}
			$out[] = $entry;
		}
		return $out;
	...
	private static function list_local_templates( $args ) {
		...
		foreach ( $posts as $tpl ) {
			...
			$templates[] = [
				'id'            => (int) $tpl->ID,
				'name'          => $tpl->post_title,
				'type'          => ( is_string( $meta_type ) && '' !== $meta_type ) ? $meta_type : null,
	...
	private static function import_template( $args ) {
		...
		$outline = self::build_outline( $elements, 0 );
	...
	private static function add_widget( $args ) {
		...
		$added_settings = ( isset( $new_element['settings'] ) && is_array( $new_element['settings'] ) )
			? array_keys( $new_element['settings'] )
			: [];
		self::record_edit_trail( $post_id, (string) $new_element['id'], 'add_widget', $added_settings );
		...
		return self::with_invalidation( self::with_revision( $response, $rev ), $inval );
	}
	...
	private static function list_widget_types( $args ) {
		if ( ! class_exists( '\Elementor\Plugin' ) || ! isset( \Elementor\Plugin::$instance ) ) {
	...
	private static function get_widget_type_schema( $args ) {
		...
		$state = self::classify_widget_type( $widget_type );
		if ( 'atomic' === $state ) {
		...
		if ( 'registered' !== $state ) {
			throw new \Exception( self::unregistered_widget_message( $widget_type, self::detect_provider_capability() ) );
		}
		...
		if ( null === $stored ) {
			$response = array_merge(
				...
				\More_MCP\Integrations\Write_Verify::report(
					...
					__( 'kit settings', 'more-mcp' ),
					__( 'The kit document could not be re-resolved after the save, so the stored values were never read. The save itself committed and the undo token is valid.', 'more-mcp' )
				)
			);
			if ( ! empty( $inval['warnings'] ) ) {
				$response['cache_invalidation'] = [
	...
	private static function set_template_conditions( $args ) {
		$post_id = (int) ( $args['post_id'] ?? 0 );
		if ( $post_id <= 0 ) {
			throw new \Exception( 'post_id is required.' );
		}
		...
		if ( ! array_key_exists( 'conditions', $args ) || ! is_array( $args['conditions'] ) ) {
			throw new \Exception( 'conditions is required and must be an array of condition strings (e.g. ["include/general"]). Pass an empty array to clear.' );
		}

		
		
		if ( ! class_exists( '\ElementorPro\Modules\ThemeBuilder\Module' ) ) {
			throw new \Exception( 'Elementor Pro Theme Builder is not available on this site. Display conditions require Elementor Pro (or a fork that ships the theme-builder module).' );
		}
		if ( 'elementor_library' !== get_post_type( $post_id ) ) {
			throw new \Exception( 'Post must be an elementor_library template (a Theme Builder document such as header/footer/single/archive).' );
		}

		$module = \ElementorPro\Modules\ThemeBuilder\Module::instance();
		if ( ! $module || ! method_exists( $module, 'get_conditions_manager' ) ) {
			throw new \Exception( 'Elementor Pro Theme Builder module is present but its conditions manager could not be reached.' );
		}
		...
		if ( ! $document ) {
			throw new \Exception( 'Post ' . esc_html( (string) $post_id ) . ' is not a Theme Builder document. Set its template type first (e.g. via elementor_import_template with a header/footer/single/archive type, or elementor_sync_library_type).' );
		}
		...
		$conditions_arg = [];
		foreach ( $args['conditions'] as $cond ) {
			$cond = trim( (string) $cond );
			if ( '' === $cond ) {
				continue;
			}
			$segments = array_values( array_filter( explode( '/', $cond ), function ( $s ) {
				return '' !== $s;
			} ) );
			if ( ! empty( $segments ) ) {
				$conditions_arg[] = $segments;
			}
		}
		...
		$before = get_post_meta( $post_id, '_elementor_conditions', true );
		...
		$cache_error = null;
		try {
			$manager->save_conditions( $document->get_main_id(), $conditions_arg );
		} catch ( \Throwable $e ) {
		...
		$stored = get_post_meta( $post_id, '_elementor_conditions', true );
		...
		$expected = [];
		foreach ( $conditions_arg as $segments ) {
			$expected[] = rtrim( implode( '/', $segments ), '/' );
		}

		$matches = ( $stored === $expected );

		$result = [
			'success'        => ( $matches && null === $cache_error ),
			'post_id'        => $post_id,
			'conditions'     => $stored,
			
			
			
			'cache_rebuilt'  => ( null === $cache_error ),
			'cleared'        => empty( $conditions_arg ),
			
			
			
			
			'unchanged'      => ( $matches && $before === $stored ),
		];
		if ( null !== $cache_error ) {
			$result['warning'] = 'Elementor Pro reported an error while saving conditions or rebuilding the cache: ' . $cache_error . '. The template may not render at its location until conditions are re-saved.';
		} elseif ( ! $matches ) {
			$result['warning'] = 'Elementor Pro accepted the call but the stored conditions do not match what was requested. Requested: '
				. esc_html( implode( ', ', $expected ) ) . '. Stored: ' . esc_html( implode( ', ', $stored ) )
				. '. The template may not render where you expect.';
		}
		return $result;
	}
}
