...
class Runtime {
	...
	private static function set_template_conditions( $args ) {
		...
		$cache_error = null;
		try {
			$saved = $manager->save_conditions( $document->get_main_id(), $conditions_arg );
		} catch ( \Throwable $e ) {
		...
		$result = [
			'success'        => ( false !== $saved && null === $cache_error ),
			'post_id'        => $post_id,
			'conditions'     => array_values( $stored ),
			'cache_rebuilt'  => ( false !== $saved && null === $cache_error ),
