...
class Code {
	...
	public static function create_code( $args ) {
		...
		$state = Write_Verify::state_of(
			(string) get_post_meta( $post_id, self::META_CODE, true ),
			$code,
			Write_Verify::NO_PRIOR
		);
		$verify = Write_Verify::report(
			$state,
			__( 'code body', 'more-mcp' ),
			__( 'Elementor sanitizes this meta key unless the caller holds unfiltered_html; compare the stored body before publishing the snippet.', 'more-mcp' )
		);
	...
	public static function update_code( $args ) {
		...
		if ( null !== $new_code ) {
			$states[] = Write_Verify::state_of(
				(string) get_post_meta( $id, self::META_CODE, true ),
				$new_code,
				$prior['code']
			);
		}
		if ( null !== $new_location ) {
			$states[] = Write_Verify::state_of( self::read_location( $id ), $new_location, $prior['location'] );
		}
		if ( null !== $new_priority ) {
			$states[] = Write_Verify::state_of( self::read_priority( $id ), $new_priority, $prior['priority'] );
		}
		if ( null !== $new_title ) {
			$fresh    = get_post( $id );
			$states[] = Write_Verify::state_of(
		...
		Write_Verify::refuse_if_discarded( $state, __( 'Custom Code snippet', 'more-mcp' ) );
		$verify = Write_Verify::report(
			$state,
			__( 'Custom Code snippet', 'more-mcp' ),
			__( 'Elementor sanitizes the code meta key unless the caller holds unfiltered_html; re-read the snippet before publishing it.', 'more-mcp' )
		);
	...
	public static function delete_code( $args ) {
		...
		$state = Write_Verify::state_of_presence( ! get_post( $id ) );
		Write_Verify::refuse_if_discarded(
			$state,
			__( 'Custom Code deletion', 'more-mcp' ),
			__( 'The Custom Code post is still present.', 'more-mcp' )
		);
		$verify = Write_Verify::report( $state, __( 'Custom Code deletion', 'more-mcp' ) );
