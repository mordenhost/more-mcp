...
class BBPress {
	...
	public static function get_tools() {
		...
		return array(
			...
			array(
				'name'        => 'bbp_get_topic_tags',
				'description' => 'Read the bbPress topic-tag taxonomy: each tag\'s id, name, slug, and usage count (how many topics carry it), plus the topic moderation breakdown (open/closed/spam/pending/trash counts from bbPress\'s own topic-status vocabulary). Tag names and slugs are operator-facing taxonomy labels, not discussion content. Returns taxonomy structure and counts only, never a topic, reply, post body, or author identity. Read-only diagnostic.',
				'inputSchema' => array(
					'type'       => 'object',
					'properties' => array(
						'limit' => array(
							'type'        => 'integer',
							'description' => 'Maximum tags to list, ordered by most used (1-200). Default 50.',
						),
					),
				),
			),
	...
	public static function execute_tool( $name, $args ) {
		...
		if ( 'bbp_get_topic_tags' === $name ) {
			return self::get_topic_tags( $args );
		}
	...
	private static function get_topic_tags( $args ) {
		if ( ! function_exists( 'bbp_get_topic_tag_tax_id' ) ) {
			return array(
				'provider'  => 'bbpress',
				'available' => false,
				'message'   => 'The bbPress topic-tag taxonomy accessor was not found.',
			);
		}
		...
		if ( is_wp_error( $tags ) ) {
			return array(
				'provider'  => 'bbpress',
				'available' => false,
				'message'   => 'The bbPress topic-tag taxonomy could not be read: ' . esc_html( $tags->get_error_message() ),
			);
		}

		$topic_statuses = function_exists( 'bbp_get_topic_statuses' ) ? bbp_get_topic_statuses() : array();
		$topic_pt       = function_exists( 'bbp_get_topic_post_type' ) ? bbp_get_topic_post_type() : null;

		
		
		$by_status = array();
		if ( null !== $topic_pt ) {
			$counts = wp_count_posts( $topic_pt );
			if ( is_object( $counts ) ) {
				foreach ( get_object_vars( $counts ) as $status => $n ) {
					...
					$label = isset( $topic_statuses[ $status ] ) ? $topic_statuses[ $status ] : $status;
					$by_status[ sanitize_key( $label ) ] = array(
						'count' => $n,
						'label' => $label,
					);
				}
