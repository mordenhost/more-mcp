...
class AutomatorWP {
	...
	private static function get_automations( $args ) {
		...
		if ( ! self::table_exists( 'automations' ) ) {
			throw new \Exception( 'The AutomatorWP automations table does not exist (the plugin has not completed setup).' );
		}
		...
		$table = self::table_name( 'automations' );
		$where = ' WHERE 1=1';
		if ( '' !== $status ) {
		...
		$rows  = $wpdb->get_results( 
			"SELECT id, title, type, status, times, times_per_user, sequential, date FROM {$table}{$where} ORDER BY id DESC LIMIT {$per_page} OFFSET {$offset}"
		);
		...
		return array(
			...
			'returned'   => count( $automations ),
			'has_more'   => ( $page - 1 ) * $per_page + count( $automations ) < $total,
