	...
	public static function get_tools() {
		...
		return array(
			...
			array(
				...
				'description' => 'Read Charitable campaign health: campaign counts by status, and optionally a bounded list of campaigns carrying only id, title, status, and publish date. Returns counts and campaign labels only — never a campaign\'s body content, goal configuration, or media, and never donor or donation data. Read-only diagnostic; cannot create, edit, or delete a campaign.',
				'inputSchema' => array(
					'type'       => 'object',
					'properties' => array(
						'include_list' => array(
							'type'        => 'boolean',
							'description' => 'When true, include a list of campaigns (id/title/status/date only, no body or goal configuration). Default false (counts only).',
						),
						'limit'        => array(
							'type'        => 'integer',
