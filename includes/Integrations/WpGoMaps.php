...
class WpGoMaps {
	...
	public static function get_tools(): array {
		...
		return [
			...
			[
				'name'        => 'wpgm_get_markers_aggregate',
				'description' => 'Aggregate WP Go Maps marker counts grouped by map_id, split by the approved flag (approved=1 default vs 0 awaiting approval). Reads the {prefix}wpgmza markers table ($WPGMZA_TABLE_NAME_MARKERS) with COUNT-GROUP-BY only — never a marker row, so address/coordinate data about real-world places is never surfaced. Read-only.',
				'inputSchema' => [
					'type'       => 'object',
					'properties' => new \stdClass(),
				],
	...
	public static function execute_tool( $name, $args ) {
		...
		switch ( $name ) {
			...
			case 'wpgm_get_markers_aggregate':
				return self::get_markers_aggregate();
			default:
	...
	private static function get_status(): array {
		...
		$rows = $wpdb->get_results( "SELECT id, map_title, map_start_lat, map_start_lng, map_start_zoom, active FROM {$maps_table} ORDER BY id ASC" ); 
		if ( ! is_array( $rows ) ) {
			$rows = array();
		}
		$by_id = array();
	...
	private static function counts_by_map( string $table, $flag_column = null ) {
		...
		$allowed     = array( 'approved' );
		$flag_column = in_array( $flag_column, $allowed, true ) ? $flag_column : null;
		$flag_sql = $flag_column ? ", {$flag_column}" : '';
		
		$rows     = $wpdb->get_results( "SELECT map_id, COUNT(*) AS n{$flag_sql} FROM {$table} WHERE map_id IS NOT NULL GROUP BY map_id" . ( $flag_column ? ", {$flag_column}" : '' ) ); 
	...
	private static function get_markers_aggregate(): array {
		global $wpdb;

		$table = null;
		if ( isset( $GLOBALS['WPGMZA_TABLE_NAME_MARKERS'] ) && is_string( $GLOBALS['WPGMZA_TABLE_NAME_MARKERS'] ) ) {
			$table = $GLOBALS['WPGMZA_TABLE_NAME_MARKERS'];
		} else {
			$table = $wpdb->prefix . 'wpgmza';
		}

		if ( ! self::table_exists( $table ) ) {
			return array( 'provider' => 'wp-go-maps', 'available' => false, 'message' => 'The markers table does not exist.', 'maps' => array() );
		}

		$rows = $wpdb->get_results(
			"SELECT map_id, approved, COUNT(*) AS n FROM {$table} WHERE map_id IS NOT NULL GROUP BY map_id, approved"
		);
		$rows = array_map( fn( $r ) => (array) $r, (array) $rows );
		...
		foreach ( $rows as $r ) {
			$map_id = (int) $r['map_id'];
			if ( ! isset( $by_map[ $map_id ] ) ) {
				$by_map[ $map_id ] = array( 'total' => 0, 'approved' => 0, 'unapproved' => 0 );
			}
			$n = (int) $r['n'];
			$by_map[ $map_id ]['total'] += $n;
			if ( (int) $r['approved'] === 1 ) {
				$by_map[ $map_id ]['approved'] += $n;
			} else {
				$by_map[ $map_id ]['unapproved'] += $n;
			}
			$total += $n;
		}

		$maps = array();
		foreach ( $by_map as $map_id => $counts ) {
			$maps[] = array(
				'map_id'     => $map_id,
				'total'      => $counts['total'],
