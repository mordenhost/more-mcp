...
class Bookly {
	...
	public static function get_tools() {
		...
		return array(
			...
			array(
				'name'        => 'bookly_get_staff_catalog',
				'description' => 'Read the Bookly staff catalogue: staff members (id, full name, visibility: public/private/archive, position), ordered by position, at most 100 per call. Returns the DEFINITIONS of who can take bookings only — the staff table\'s email/phone and calendar/Zoom credential columns are never selected. Customer and booking data is never returned. Read-only; cannot modify anything.',
				'inputSchema' => array(
					'type'       => 'object',
					'properties' => array(
						'limit' => array(
	...
	public static function execute_tool( $name, $args ) {
		...
		if ( 'bookly_get_staff_catalog' === $name ) {
	...
	private static function get_staff_catalog( $args ) {
		global $wpdb;
		$out = array();
		if ( ! self::table_exists( self::STAFF_TABLE ) ) {
			return array(
				'provider'    => 'bookly',
				'available'   => false,
				'total'       => 0,
		...
		$table = self::table_name( self::STAFF_TABLE );
		$rows  = $wpdb->get_results(
			$wpdb->prepare( "SELECT id, full_name, visibility, position FROM {$table} ORDER BY position ASC, id ASC LIMIT %d", $limit ),
			ARRAY_A
		);
		if ( is_array( $rows ) ) {
			foreach ( $rows as $row ) {
