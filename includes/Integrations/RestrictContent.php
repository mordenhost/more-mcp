...
class RestrictContent {
	...
	public static function get_tools() {
		...
		return array(
			...
			array(
				'name'        => 'rcp_get_discounts',
				'description' => 'List the Restrict Content discount-code catalogue: each discount\'s name, code, amount with its unit (percent or flat), use count against its max uses, whether it is enabled, its expiration date, whether it applies only to initial payments (one-time), and the membership-level ids it is locked to. Read from the plugin\'s own discounts table (the same store RCP_Discount reads). Discount definitions only — never a membership, customer, or payment record. Read-only.',
				'inputSchema' => array(
					'type'       => 'object',
					'properties' => array(
						'limit' => array(
							'type'        => 'integer',
							'description' => 'Maximum discounts to return (1-50, default 25).',
						),
					),
				),
			),
	...
	public static function execute_tool( $name, $args ) {
		...
		if ( 'rcp_get_discounts' === $name ) {
			return self::get_discounts( $args );
		}
	...
	private static function get_discounts( $args ) {
		global $wpdb;
		$table = self::table( 'rcp_discounts' );

		if ( ! self::table_exists( $table ) ) {
			return array(
				'provider'  => 'restrictcontent',
				'available' => false,
				'message'   => 'The Restrict Content discounts table was not found.',
			);
		}
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT id, name, amount, unit, code, use_count, max_uses, status, expiration, one_time, membership_level_ids FROM {$table} ORDER BY id DESC LIMIT %d",
				$limit
			),
			ARRAY_A
		);
		...
		if ( is_array( $rows ) ) {
			foreach ( $rows as $row ) {
				$level_ids = maybe_unserialize( $row['membership_level_ids'] );
				...
				$discounts[] = array(
					...
					'enabled'              => isset( $row['status'] ) && 'active' === (string) $row['status'],
					'expiration'           => ! empty( $row['expiration'] ) ? (string) $row['expiration'] : null,
					'one_time'             => ! empty( $row['one_time'] ),
					'membership_level_ids' => array_map( 'intval', array_values( $level_ids ) ),
				);
			}
		}
