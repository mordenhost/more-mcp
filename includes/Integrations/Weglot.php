	...
	public static function get_tools() {
		...
		return array(
			array(
				...
				'description' => 'Read Weglot multilingual configuration: the original (source) language, the configured destination languages with each one\'s language code, custom code/slug, and whether it is public, and whether Weglot is connected (an API key is configured). Weglot translates on the fly through its cloud service rather than storing one WordPress post per language, so it offers site-wide configuration only — no per-object language resolution or translation-sibling lookup. The API key is never returned. Read-only.',
				'inputSchema' => array(
					'type'       => 'object',
