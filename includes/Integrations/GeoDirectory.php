...
class GeoDirectory {
	...
	public static function get_tools() {
		...
		return array(
			...
			array(
				'name'        => 'geodir_get_categories',
				'description' => 'List the categories of a GeoDirectory listing post type (its {post_type}category taxonomy): each term\'s id, name, slug, listing count, and parent term id. Category DEFINITIONS only — no listing values, no owner identity. Read-only taxonomy enumeration.',
				'inputSchema' => array(
					'type'       => 'object',
					'properties' => array(
						'post_type' => array( 'type' => 'string', 'description' => 'A GeoDirectory listing post type (e.g. gd_place). Defaults to the first post type GeoDirectory registers, falling back to gd_place.' ),
					),
				),
			),
		);
	...
	public static function execute_tool( $name, $args ) {
		...
		if ( 'geodir_get_categories' === $name ) {
			$post_type = isset( $args['post_type'] ) ? sanitize_key( $args['post_type'] ) : self::default_post_type();
			return self::get_categories( $post_type );
		}
	...
	private static function default_post_type() {
		$types = self::post_types();
		return isset( $types[0] ) ? $types[0] : 'gd_place';
	}

	private static function get_categories( $post_type ) {
		if ( '' === $post_type ) {
			throw new \Exception( 'post_type is required.' );
		}

		$taxonomy = self::resolve_category_taxonomy( $post_type );
		if ( null === $taxonomy ) {
	...
	private static function resolve_category_taxonomy( $post_type ) {
		$taxonomy = null;
		if ( function_exists( 'geodir_get_taxonomies' ) ) {
			$taxonomies = geodir_get_taxonomies( $post_type, false );
		...
		if ( null === $taxonomy ) {
			$fallback = $post_type . 'category';
			if ( function_exists( 'taxonomy_exists' ) && taxonomy_exists( $fallback ) ) {
				$taxonomy = $fallback;
			}
		}
		if ( null === $taxonomy ) {
