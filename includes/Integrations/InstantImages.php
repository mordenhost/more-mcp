...
class InstantImages {
	...
	public static function get_tools() {
		...
		$tools   = array();
		$tools[] = array(
			'name'        => 'stock_get_provider_settings',
			'description' => 'Read Instant Images provider configuration: every provider (name, slug, whether it requires an API key, its wp-config constant), whether each is active, and whether each has an API key configured (a boolean — the key VALUE is never returned). Also surfaces the global download/attribution settings. Read-only. If no provider has a key, use this to learn what is missing.',
			'inputSchema' => array(
				'type'       => 'object',
	...
	private static function get_provider_settings() {
		$providers = array();
		if ( class_exists( '\InstantImages' ) && method_exists( '\InstantImages', 'instant_img_get_providers' ) ) {
		...
		$active = array();
		if ( class_exists( '\InstantImages' ) && method_exists( '\InstantImages', 'instant_img_get_active_providers' ) ) {
			$active = \InstantImages::instant_img_get_active_providers();
		}
		$active_map = array();
		foreach ( (array) $active as $slug ) {
			$active_map[ (string) $slug ] = true;
		}

		$api_settings = get_option( 'instant_img_api_settings', array() );
		...
		foreach ( $providers as $p ) {
			...
			if ( isset( $api_settings[ $opt_key ] ) && is_string( $api_settings[ $opt_key ] ) && '' !== trim( $api_settings[ $opt_key ] ) ) {
				$configured = true;
			}
			
			$constant         = isset( $p['constant'] ) ? (string) $p['constant'] : '';
			$constant_defined = ( '' !== $constant && defined( $constant ) && '' !== (string) constant( $constant ) );
			if ( $constant_defined ) {
				$configured = true;
			}
			$list[] = array(
				'name'              => isset( $p['name'] ) ? (string) $p['name'] : $slug,
				'slug'              => $slug,
				'requires_key'      => ! empty( $p['requires_key'] ),
				'active'            => isset( $active_map[ $slug ] ),
		...
		$general = get_option( 'instant_img_settings', array() );
		if ( ! is_array( $general ) ) {
			$general = array();
		}

		return array(
			'provider' => 'instant-images',
			'providers' => $list,
			'global'   => array(
				'max_width'        => isset( $general['unsplash_download_w'] ) ? (int) $general['unsplash_download_w'] : 1600,
				'max_height'       => isset( $general['unsplash_download_h'] ) ? (int) $general['unsplash_download_h'] : 1200,
				'auto_attribution' => ! empty( $general['auto_attribution'] ),
				'media_modal_display' => ! empty( $general['media_modal_display'] ),
			),
		);
	}
	...
	public static function execute_tool( $name, $args ) {
		...
		if ( 'stock_get_provider_settings' === $name ) {
