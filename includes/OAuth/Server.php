...
class Server {
    ...
    private function register( $request_method = 'GET' ) {
        ...
        $blocked = $this->throttle_by_ip( 'more_mcp_reg_rate_', 10, 60 );
        if ( is_int( $blocked ) ) {
            $this->json_error(
                'rate_limit',
    ...
    private function token( $request_method = 'GET' ) {
        ...
        $blocked = $this->throttle_by_ip( 'more_mcp_token_rate_', 60, 60 );
        if ( is_int( $blocked ) ) {
            $this->json_error(
                'rate_limit',
    ...
    private function json_error( $error, $description, $status = 400, $extra_headers = [] ) {
    ...
    private function throttle_by_ip( $key_prefix, $max, $window ) {
        ...
        return max( 1, $window - ( $now - (int) $data['start'] ) );
    }
