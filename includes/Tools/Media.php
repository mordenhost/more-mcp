...
class Media implements Handler {
	...
	public static function get_tools(): array {
		return [
			...
			['name' => 'wp_process_image', 'description' => 'Process an existing image attachment with WordPress\'s built-in image editor and save the result as a NEW attachment (the original is never modified or deleted). Supports resize (bounded to max width/height, aspect ratio preserved unless crop=true) and format conversion (webp, jpeg, png). Use to produce a web-optimized or correctly-sized copy of a library image without an external service or API key. Returns the new attachment id and url. Requires upload_files plus edit_post on the source attachment.', 'inputSchema' => ['type' => 'object', 'properties' => ['id' => ['type' => 'integer', 'description' => 'Source image attachment ID.'], 'max_width' => ['type' => 'integer', 'description' => 'Resize so width does not exceed this (px). Omit to keep width.'], 'max_height' => ['type' => 'integer', 'description' => 'Resize so height does not exceed this (px). Omit to keep height.'], 'crop' => ['type' => 'boolean', 'description' => 'When resizing with both dimensions, hard-crop to exactly max_width x max_height instead of fitting within. Default false (fit, preserve aspect ratio).'], 'format' => ['type' => 'string', 'enum' => ['webp', 'jpeg', 'jpg', 'png'], 'description' => 'Convert the output to this format. Omit to keep the source format.'], 'quality' => ['type' => 'integer', 'description' => 'Output quality 1-100 for lossy formats (webp/jpeg). Omit for WordPress default.'], 'title' => ['type' => 'string', 'description' => 'Title for the new attachment. Defaults to the source title with a suffix.'], 'alt_text' => ['type' => 'string', 'description' => 'Alt text for the new attachment. Defaults to the source alt text.']], 'required' => ['id']]],
	...
	public static function supports( string $name ): bool {
		static $names = [
			...
			'wp_set_featured_image', 'wp_update_media', 'wp_process_image', 'wp_delete_media', 'wp_count_media',
		];
	...
	private static function process_image( array $args ) {
		$id  = self::resolve_post_id_arg( $args );
		$src = $id > 0 ? get_post( $id ) : null;
		if ( ! $src || 'attachment' !== $src->post_type ) {
		...
		if ( 0 !== strpos( (string) $src->post_mime_type, 'image/' ) ) {
			throw new \Exception( 'Attachment ' . intval( $id ) . ' is not an image (' . esc_html( (string) $src->post_mime_type ) . ').' );
		}

		$max_width  = isset( $args['max_width'] ) ? (int) $args['max_width'] : 0;
		$max_height = isset( $args['max_height'] ) ? (int) $args['max_height'] : 0;
		$crop       = ! empty( $args['crop'] );
		$format     = isset( $args['format'] ) ? strtolower( sanitize_key( $args['format'] ) ) : '';
		$quality    = isset( $args['quality'] ) ? (int) $args['quality'] : 0;

		if ( $max_width <= 0 && $max_height <= 0 && '' === $format ) {
			throw new \Exception( 'Nothing to do: provide max_width/max_height to resize and/or format to convert.' );
		}

		$new_id = Media_Support::process_attachment_to_new(
			$id,
			array(
				'max_width'  => $max_width,
		...
		return array(
			...
			'message'   => 'Processed image saved as a new attachment; the original was left unchanged.',
		);
	}
}
