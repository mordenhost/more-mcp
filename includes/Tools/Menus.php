...
class Menus implements Handler {
	...
	public static function get_tools(): array {
		return [
			...
			['name' => 'wp_create_menu', 'description' => 'Create a new navigation menu (nav_menu term) and return its menu_id, so the item tools have something to populate — wp_create_menu_item requires a menu_id that must already exist, and a fresh site has none. Optionally assign the new menu to one or more registered theme locations (e.g. "primary", "footer") so it renders in the theme without a manual Appearance → Menus step. Refuses a duplicate menu name (WordPress requires nav-menu names to be unique). Requires edit_theme_options capability.', 'inputSchema' => ['type' => 'object', 'properties' => ['name' => ['type' => 'string', 'description' => 'Human-readable menu name (must be unique among existing menus).'], 'locations' => ['type' => 'array', 'items' => ['type' => 'string'], 'description' => 'Optional registered theme-location slugs to assign this menu to (from the active theme). Unknown slugs are reported back in unknown_locations and skipped, not fatal.']], 'required' => ['name']]],
	...
	public static function supports( string $name ): bool {
		static $names = [
			'wp_get_menus', 'wp_create_menu', 'wp_get_menu_items', 'wp_create_menu_item',
	...
	public static function execute_tool( string $name, array $args ) {
		switch ( $name ) {
			...
			case 'wp_create_menu':
				...
				if ($menu_name === '') {
					throw new \Exception('A non-empty menu name is required.');
				}

				
				if (wp_get_nav_menu_object($menu_name)) {
					throw new \Exception('A menu named "' . esc_html($menu_name) . '" already exists. Menu names must be unique.');
				}
				...
				$assigned  = [];
				$unknown   = [];
				$requested = isset($args['locations']) && is_array($args['locations']) ? $args['locations'] : [];
				if (!empty($requested)) {
