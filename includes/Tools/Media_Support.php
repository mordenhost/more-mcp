...
class Media_Support {
	...
	public static function sideload_image_from_url( $url, $filename, $title, $caption, $alt_text ) {
		...
		if (!$is_local_host) {
			$verdict = \More_MCP\Platform\Registry::url_resolves_to_blocked_address($host);
			if ($verdict === \More_MCP\Platform\Registry::HOSTNAME_BLOCKED) {
				throw new \Exception('URL resolves to a blocked address range.');
			}
			if ($verdict === \More_MCP\Platform\Registry::HOSTNAME_UNRESOLVED) {
