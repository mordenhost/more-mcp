...
class Undo_Dispatcher {
    ...
    public static function dispatch( array $args ) {
        ...
        switch ($undo_op) {
            ...
            case 'cptui_data_model_write':

                

                if (!current_user_can('manage_options')) {
                    throw new \Exception('manage_options capability required to undo this Custom Post Type UI operation.');
                }
                if (!class_exists('\More_MCP\Integrations\CptUi')) {
                    throw new \Exception('The Custom Post Type UI integration is unavailable, so this change cannot be undone.');
                }
                $cptui_undo_result = \More_MCP\Integrations\CptUi::undo_data_model_write($undo_snapshot);
                \More_MCP\MCP\Undo_Store::delete($undo_token);
                return $cptui_undo_result;

            case 'toolset_data_model_write':
                ...
                if (!current_user_can('manage_options')) {
                    throw new \Exception('manage_options capability required to undo this Toolset Types operation.');
                }
                if (!class_exists('\More_MCP\Integrations\ToolsetTypes')) {
                    throw new \Exception('The Toolset Types integration is unavailable, so this change cannot be undone.');
                }
                $toolset_undo_result = \More_MCP\Integrations\ToolsetTypes::undo_data_model_write($undo_snapshot);
                \More_MCP\MCP\Undo_Store::delete($undo_token);
                return $toolset_undo_result;

            case 'mbcpt_data_model_write':
                ...
                if (!current_user_can('manage_options')) {
                    throw new \Exception('manage_options capability required to undo this MB Custom Post Types operation.');
                }
                if (!class_exists('\More_MCP\Integrations\MbCpt')) {
                    throw new \Exception('The MB Custom Post Types integration is unavailable, so this change cannot be undone.');
                }
                $mbcpt_undo_result = \More_MCP\Integrations\MbCpt::undo_data_model_write($undo_snapshot);
                \More_MCP\MCP\Undo_Store::delete($undo_token);
                return $mbcpt_undo_result;
            ...
            case 'elementor_asset_write':

                

                if (!current_user_can('edit_theme_options')) {
                    throw new \Exception('edit_theme_options capability required to undo this Elementor font/icon operation.');
                }
                $el_asset_result = \More_MCP\Integrations\Elementor::undo_asset_write($undo_snapshot);
                \More_MCP\MCP\Undo_Store::delete($undo_token);
                return $el_asset_result;

            case 'elementor_code_write':

                

                if (!current_user_can('manage_options')) {
                    throw new \Exception('manage_options capability required to undo this Elementor Custom Code operation.');
                }
                $undo_cc_settings = get_option('more_mcp_settings', array());
                if (!is_array($undo_cc_settings) || empty($undo_cc_settings['allow_code_snippets'])) {
                    throw new \Exception('Code writes are disabled, so this Elementor Custom Code change cannot be undone. Enable "Allow code snippets" first.');
                }
                $el_code_result = \More_MCP\Integrations\Elementor::undo_code_write($undo_snapshot);
                \More_MCP\MCP\Undo_Store::delete($undo_token);
                return $el_code_result;

            case 'blocks_global_styles_write':
                ...
                if (!current_user_can('edit_theme_options')) {
                    throw new \Exception('You do not have permission to edit theme options, so this global styles change cannot be undone.');
                }
                $undo_gs_pre_op = isset($undo_snapshot['pre_op_state']) && is_array($undo_snapshot['pre_op_state'])
                    ? $undo_snapshot['pre_op_state']
                    : [];
                if (!array_key_exists('styles', $undo_gs_pre_op) || !is_array($undo_gs_pre_op['styles'])) {
                    throw new \Exception('Undo snapshot has no global styles to restore.');
                }

                
                
                $undo_gs_result = \More_MCP\Blocks\Global_Styles::restore_styles(
                    $undo_gs_pre_op['styles'],
                    !empty($undo_gs_pre_op['post_existed'])
                );
                \More_MCP\MCP\Undo_Store::delete($undo_token);
                $undo_gs_response = [
                    'success'          => true,
                    'op'               => 'blocks_global_styles_write',
                    'post_id'          => isset($undo_gs_result['post_id']) ? (int) $undo_gs_result['post_id'] : 0,
                    'verified'         => !empty($undo_gs_result['verified']),
                    'message'          => 'Site global styles restored to their pre-operation state.',
                    'restored_summary' => isset($undo_snapshot['summary']) ? (string) $undo_snapshot['summary'] : '',
                ];
                if (!empty($undo_gs_result['note'])) {
                    $undo_gs_response['note'] = $undo_gs_result['note'];
                }
                if (!empty($undo_gs_result['cache_invalidation']['warnings'])) {
                    $undo_gs_response['cache_invalidation'] = $undo_gs_result['cache_invalidation'];
                }
                return $undo_gs_response;
            ...
            case 'snippet_update':
                ...
                if (!current_user_can('manage_options')) {
                    throw new \Exception('manage_options capability required to undo this code-snippets operation.');
                }
                $undo_snip_settings = get_option('more_mcp_settings', array());
                if (!is_array($undo_snip_settings) || empty($undo_snip_settings['allow_code_snippets'])) {
                    throw new \Exception('Code writes are disabled, so this code-snippets change cannot be undone. Enable "Allow code snippets" first.');
                }
                if (!class_exists('\More_MCP\Integrations\Snippets')) {
                    throw new \Exception('The Code Snippets integration is unavailable, so this change cannot be undone.');
                }
                $snippet_undo_result = \More_MCP\Integrations\Snippets::undo_write($undo_snapshot);
                \More_MCP\MCP\Undo_Store::delete($undo_token);
                return $snippet_undo_result;
