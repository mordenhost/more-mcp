<?php
/**
 * Gutenberg blocks output schemas.
 *
 * Covers both Blocks\Content (block CRUD on posts and pages) and
 * Blocks\Templates (FSE templates, patterns, reusable blocks).
 *
 * Every schema sets additionalProperties => true. The handlers attach
 * advisory keys — undo envelopes, verification notes, registry caveats —
 * that are deliberately not enumerated here, because WP_Ability::validate_output()
 * rejects unlisted keys when additionalProperties is false, and losing a
 * caveat to schema validation would be worse than leaving it undeclared.
 */
...
class Blocks {
	...
	private static function map(): array {
		// Shared shape for the summarized block rows returned by tree reads.
		$block_summary = array(
			'type'  => 'array',
			'items' => array(
				'type'                 => 'object',
		...
		// Shared shape for the undo envelope emitted by every mutating tool.
		$undo = array(
			'type'                 => 'object',
			'additionalProperties' => true,
			'properties'           => array(
		...
		return array(
			...
			// Confidence is an enum rather than a boolean on purpose: PHP
			// cannot run a block's save(), so there is no honest true/false.
			...
			// ---------- Content: mutations ----------
			// One shape for all four: dry_run returns written=false plus a
			// preview; a real write returns written=true, verification, and undo.
	...
	/**
	 * Shared response shape for the four post/page block mutations.
	 *
	 * @param array $block_summary Summarized-tree schema fragment.
	 * @param array $undo          Undo-envelope schema fragment.
	 * @return array
	 */
