<?php
/**
 * Gutenberg block CRUD on posts and pages.
 *
 * Read tools return a block tree annotated with index paths; mutation tools
 * take a path and change exactly one block. Every mutation follows the same
 * discipline as the existing replace_in_post_content() helper in MCP/Server.php:
 *
 *   - object-level capability check first
 *   - operate on RAW stored post_content, never filtered output
 *   - dry_run previews without writing
 *   - an expectation guard aborts when the target is not what the caller thinks
 *   - serialize -> reparse -> serialize self-consistency check before writing
 *   - read-after-write verification, so silent drops surface as errors
 *   - an Undo_Store snapshot on every destructive write
 *
 * On index paths: they are positional, so they shift when siblings are added
 * or removed. Every read returns current paths and every mutation returns the
 * post-mutation tree, so callers can re-anchor rather than guess.
 */
...
class Content {
	...
	/**
	 * Dispatch a blocks_* content tool.
	 *
	 * @param string $name Tool name.
	 * @param array  $args Tool arguments.
	 * @return mixed
	 * @throws \Exception On validation failure or permission denial.
	 */
	...
	/**
	 * blocks_validate_markup handler.
	 *
	 * @param array $args Tool args.
	 * @return array
	 * @throws \Exception
	...
	/**
	 * blocks_get_type_schema handler.
	 *
	 * @param array $args Tool args.
	 * @return array
	 * @throws \Exception
	...
	/**
	 * Run the write pipeline: self-consistency check, dry-run short-circuit,
	 * undo snapshot, write, read-after-write verification.
	 *
	 * @param \WP_Post $post    Target post.
	 * @param array    $tree    Mutated block tree.
	 * @param array    $args    Original tool args (for dry_run).
	 * @param string   $summary Human-readable description of the change.
	 * @param array    $extra   Extra keys to merge into the response.
	 * @return array
	 * @throws \Exception
	 */
	private static function finalize( \WP_Post $post, array $tree, array $args, string $summary, array $extra = [] ): array {
		...
		// Snapshot BEFORE writing so undo can restore the exact prior content.
		...
		$updated = wp_update_post(
			[
				'ID'           => $post->ID,
				// wp_update_post runs its args through wp_unslash, so content
				// must be slashed on the way in or backslashes are eaten.
		...
		// Read-after-write: surface silent modification rather than hiding it.
	...
	/**
	 * Build a block array from structured args or raw markup.
	 *
	 * @param array $args Tool args.
		...
		if ( ! empty( $args['markup'] ) && is_string( $args['markup'] ) ) {
			...
			$parsed = array_values(
				array_filter(
					$parsed,
					static function ( $b ) {
						// parse_blocks emits whitespace-only freeform blocks
						// between real ones; drop them so a single supplied
						// block does not arrive wrapped in empty siblings.
	...
	/**
	 * Enforce the expected_block_name guard when supplied.
	 *
	 * @param array  $blocks Block tree.
	 * @param string $path   Target path.
	 * @param array  $args   Tool args.
	 * @throws \Exception When the guard does not match.
	 */
	...
	/**
	 * Fetch a post the caller may read.
	 *
	 * @param array $args Tool args.
	 * @return \WP_Post
	 * @throws \Exception
	...
	private static function require_readable_post( array $args ): \WP_Post {
		...
		// Coarse gate first, so an unauthenticated caller cannot probe which
		// post IDs exist by distinguishing "not found" from "forbidden".
		...
		// read_post is the object-level meta cap: it accounts for draft,
		// pending, private and password-protected states, which a bare 'read'
		// check does not. These tools expose RAW post_content, so a coarse
		// check here would leak unpublished content to any subscriber.
	...
	/**
	 * Fetch a post the caller may edit.
	 *
	 * @param array $args Tool args.
	 * @return \WP_Post
	 * @throws \Exception
	...
	private static function require_editable_post( array $args ): \WP_Post {
		...
		// Object-level edit_post resolves to the post-type-specific capability
		// through map_meta_cap, matching wp_update_post / replace_in_post_content.
