	...
	public static function describe( string $capability ): array {
		$labels = array(
			...
			'affiliate'     => array(
				...
				'summary' => 'Affiliate-program scale: affiliate and commission counts by status. Read-only diagnostic.',
			),
			'ai_media'      => array(
				'label'   => 'AI media generation',
				'summary' => 'AI image generation and alt text through a configured provider (OpenAI, Google). Every call spends money; gated by the "Allow AI media" toggle.',
			),
			'automation'    => array(
				'label'   => 'Automation',
				'summary' => 'Automation recipe scale and status. Read-only diagnostic, counts and definitions only.',
			),
			'data_models'   => array(
				'label'   => 'Custom content models',
				'summary' => 'Custom post types and taxonomies registered by a data-model plugin. Read-only diagnostic, definitions only.',
			),
			'directories'   => array(
				'label'   => 'Directories',
				'summary' => 'Directory listing scale, category structure, and listing field schema. Read-only diagnostic.',
			),
			'job_board'     => array(
				'label'   => 'Job board',
				'summary' => 'Job-listing scale: job counts by status plus taxonomy totals. Read-only diagnostic.',
			),
			'mapping'       => array(
				'label'   => 'Maps',
				'summary' => 'Map inventory: maps, markers, and their definitions. Read-only diagnostic.',
			),
			'real_estate'   => array(
				'label'   => 'Real estate',
				'summary' => 'Property-listing scale and field schema. Read-only diagnostic.',
			),
			'search'        => array(
				'label'   => 'Site search',
				'summary' => 'Search-plugin health: index state and scale, query-log statistics, and configured search forms. Read-only diagnostic.',
			),
			'snippets'      => array(
				'label'   => 'Code snippets',
				'summary' => 'Code-snippet definitions (id, name, scope, type, active and locked state), never a code body. Write tools additionally require the "Allow code snippets" toggle and refuse PHP.',
			),
			'social_publishing' => array(
				'label'   => 'Social publishing',
				'summary' => 'Social-publishing connection state and scheduled vs published post counts. Read-only diagnostic.',
			),
		);
