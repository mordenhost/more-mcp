...
| Tool count | 107 always registered; more when integrations and lifecycle tools are enabled |
...
Yes. When WooCommerce is active **and** you enable the WooCommerce integration under More MCP → Settings → Permissions → Plugins, More MCP adds 29 MCP tools spanning product management (simple and variable, including variation CRUD and global attribute management), full coupon management (list/get/create/update/delete + bulk trash purge), order management (view, update status), customer data, and store statistics. The integration is off until you enable it — activating WooCommerce alone does not expose the tools.
