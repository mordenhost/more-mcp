...
<p align="center"><strong>Give AI assistants a safe, authenticated way into WordPress.</strong><br>Self-hosted OAuth and API keys, per-tool capability checks, audit logging, and 111 always-on tools plus opt-in integrations.</p>
...
A standard installation exposes 111 always-on tools for WordPress core, Gutenberg blocks, and outbound webhooks. Compatible plugins can add specialized tools once an administrator enables their integration, while ten high-impact plugin and theme lifecycle tools remain disabled until an administrator explicitly enables them. The live count depends on the active integrations and permissions and is shown in the Documentation panel, which reads the registry directly; every tool is reachable through the native MCP endpoint, the classic REST API, and the WordPress Abilities API on WordPress 6.9 or later.
...
| Tool count | 111 always registered; 10 opt-in lifecycle tools; plus opt-in integration tools (live count shown in the Documentation panel) |
...
Every installation registers **111 always-on tools**: 87 WordPress core tools, 21 Gutenberg block tools, and 3 outbound-webhook tools (none are gated on a third-party plugin). Opt-in integrations add more tools once an administrator enables them (WooCommerce alone adds 29), and ten plugin/theme lifecycle tools appear only when an administrator enables them. Exact live counts are shown in the Documentation panel, which reads the registry directly.
...
More MCP detects compatible plugins and can expose specialized MCP tools for them. Each integration is **off by default and requires two things** before its tools appear: the host plugin must be active, **and** an administrator must enable that integration under **More MCP → Settings → Access → Plugins**. This opt-in is a deliberate safety gate — activating a plugin never silently widens the agent's reach until you turn the integration on. The toggle lives in the denylisted `more_mcp_settings` option, so it is set from the admin screen, not through MCP itself.

The live integration catalogue is pinned by `tests/integration-toggles-test.php` (79 integrations at the time of writing); the per-integration tool counts below come from each integration's `get_tools()`. The maximum integration tool count depends on which plugins are installed and enabled and whether Elementor Pro modules are active — a ceiling, not a default. The live count is whatever the detected and enabled integrations actually contribute, and is shown directly in the Documentation panel.
...
When Meta Box is active, AI agents can read and write custom fields the same way the Meta Box UI does — rather than the raw serialized values WordPress meta returns:

- Read a single custom field with the Meta Box formatting applied
- Read every custom field on a post in one call, with name/label/type/value bundled
- Update a custom field with type-aware value handling
- Enumerate Meta Box field groups on the site for discovery before reading or writing

**Cache Integration (6 providers, 15 tools):**
When a supported cache plugin is active, AI agents can purge cached content after a write so visitors do not see stale pages:

- **LiteSpeed Cache** (2 tools) — purge all or a specific URL. Purge is driven through LiteSpeed's public `litespeed_purge_all` / `litespeed_purge_url` action hooks, not its internal classes, so the integration survives the plugin's internal refactors. No cache-stats tool is exposed because LiteSpeed has no stable public accessor for a file/size count across its disk, object-cache, and CDN layers
- **WP Rocket** (3 tools) — purge all, purge a URL, or purge minified/optimized assets
- **W3 Total Cache** (4 tools) — purge all, purge a URL, purge minified assets, and read cache status
- **Redis Object Cache** (2 tools) — read cache status and flush the object cache
- **Autoptimize** (2 tools) — purge all and read cache stats
- **WP-Optimize** (2 tools) — purge the page cache and read cache status

Purges drive each plugin's own public API, never direct file or option deletion, so the integration survives the plugin's internal refactors.
...
When Site Kit by Google, Jetpack Stats, MonsterInsights, or Burst Statistics is active, AI agents can read normalized analytics status and reports through the installed plugin — never by owning vendor credentials or calling a vendor API directly:
...
When Gravity Forms, Fluent Forms, Contact Form 7, WPForms, Ninja Forms, or Elementor Pro Forms is active, AI agents can work with forms and submissions through one normalized surface — the same tool shape regardless of which form plugin is installed (read-only where a provider stores no entries):
...
When WP Mail SMTP, Easy WP SMTP, FluentSMTP, or Post SMTP is active, AI agents can read the site's outgoing-email configuration status:

- Read the active mailer slug, whether setup is complete, the non-secret From name and From email, and the last send-error summary when the plugin exposes one
- Credentials, API keys, and passwords are never returned. FluentSMTP is read from its raw option so its own decrypting helper is never called

**Security Integration (6 total, 18 tools, read-only):**
When a supported security plugin is active, AI agents can read security status — no secret is ever read or returned:

- **Wordfence** (5 tools) — security status, scan results, blocked IPs, and related diagnostics
- **WP Defender** (5 tools) — scan results, scan status, blocked IPs, and related diagnostics
- **Solid Security** (3 tools) — security status, findings, and lockouts
- **Sucuri Security** (1 tool) — security status
- **Really Simple Security** (2 tools) — security status and vulnerability feed
- **Akismet** (2 tools) — spam status and comment check

**Backup Integration (5 total, 12 tools, read-only):**
When a supported backup plugin is active, AI agents can read backup status — package hashes, local storage paths, and log paths are never returned:

- **UpdraftPlus** (4 tools) — list backup sets, last backup summary, backup status, and start a backup
- **BackWPup** (2 tools) — list jobs and read a job
- **Duplicator** (2 tools) — list packages and read status
- **WPvivid** (2 tools) — list backups and read status
- **All-in-One WP Migration** (2 tools) — list backups and read status

**CRM Integration (3 total, 6 tools, read-only):**
When a supported CRM plugin is active, AI agents can read marketing and contact data:

- **FluentCRM** (2 tools) — status and campaign list
- **MailPoet** (2 tools) — status and campaign list
- **Groundhogg** (2 tools) — status and campaign list

**Membership Integration (4 total, 8 tools, read-only):**
When a supported membership plugin is active, AI agents can read membership structure:

- **Paid Memberships Pro** (2 tools) — status and membership levels
- **Simple Membership** (2 tools) — status and membership levels
- **ProfilePress** (2 tools) — status and subscription plans
- **Restrict Content** (2 tools) — status and membership levels

**Donations Integration (2 total, 4 tools, read-only):**
When a supported donation plugin is active, AI agents can read giving data:

- **GiveWP** (2 tools) — status and campaigns
- **Charitable** (2 tools) — status and campaigns

**Booking Integration (4 total, 8 tools, read-only):**
When a supported booking plugin is active, AI agents can read service and availability data:

- **Amelia** (2 tools) — status and service catalog
- **LatePoint** (2 tools) — status and service catalog
- **Bookly** (2 tools) — status and service catalog
- **BookingPress** (2 tools) — status and service catalog

**LMS Integration (5 total, 10 tools, read-only):**
When a supported LMS plugin is active, AI agents can read course structure:

- **LearnPress** (2 tools) — status and course structure
- **Sensei LMS** (2 tools) — status and course structure
- **Masteriyo LMS** (2 tools) — status and course structure
- **Tutor LMS** (2 tools) — status and course structure
- **LifterLMS** (2 tools) — status and course structure

**Events Integration (3 total, 6 tools, read-only):**
When a supported events plugin is active, AI agents can read events with hydrated detail:

- **The Events Calendar** (2 tools) — list events and read one event with venue and organizer detail
- **Sugar Calendar** (2 tools) — list events and read one event
- **Events Manager** (2 tools) — list events and read one event

**Multilingual Integration (3 total, 7 tools, read-only):**
When a supported multilingual plugin is active, AI agents can list languages and resolve a post's or term's language and its translation siblings:

- **TranslatePress** (1 tool) — read the configured language set and URL slugs
- **Polylang** (3 tools) — list languages, resolve a post's or term's language, and list its translations
- **WPML** (3 tools) — list languages, resolve a post's or term's language, and list its translations

No secret is read; WPML element types are validated before any filter fires.

**E-commerce Integration (2 total, 4 tools, read-only):**
When a supported e-commerce plugin is active, AI agents can read storefront data:

- **Easy Digital Downloads** (2 tools) — status and download catalog
- **FluentCart** (2 tools) — status and product catalog

**Custom Fields / Content Models (3 total, 6 tools, read-only):**
When a supported content-modeling plugin is active, AI agents can discover and read custom content structures:

- **Pods** (2 tools) — list content models and read one model
- **Custom Post Type UI** (2 tools) — list custom post types and taxonomies
- **MB Custom Post Types** (2 tools) — list custom post types and taxonomies

**Code Snippets Integration (4 tools):**
When the Code Snippets plugin is active **and** the "Allow code snippets" toggle is on, AI agents can manage code snippets — this is the write-side safety gate that is gated on its own dedicated option rather than the shared catalogue:

- `snippet_list` and `snippet_get` are read-only
- `snippet_create` and `snippet_update` write CSS or JS only, gated behind the toggle, `manage_options`, and two-part confirmation
- PHP snippet writes are refused by name (that is remote code execution), and every created snippet is saved disabled so a human must activate it. No activate, deactivate, or delete

**AI Media Integration (2 tools, off by default):**
When the "Allow AI media generation" toggle is on, AI agents can generate images and alt text through the AI provider you already configured (OpenAI or Google):

- `ai_generate_image` and `ai_generate_alt_text` call the configured provider
- Disabled behind the toggle because every call spends provider credits; the key is never returned, requests go only to fixed provider endpoints, and nothing is exposed until you turn it on

**Instant Images Integration (1 tool, read-only):**
When Instant Images is active, AI agents can search Unsplash and Pexels for stock photos using the API keys the plugin already stores:

- `stock_search_images` returns candidates with license and required attribution, ready to hand to `wp_upload_media_from_url`. No new credential store

**Image Optimizer Integration (4 total, 4 tools, read-only):**
When a supported image optimizer is active, AI agents can read optimization status:

- **Imagify** (1 tool) — optimization status
- **EWWW Image Optimizer** (1 tool) — optimization status (bytes saved, counts, average compression)
- **WP Smush** (1 tool) — optimization status
- **ShortPixel** (1 tool) — optimization status

Where a vendor exposes no stable public accessor for a figure, that figure is reported null rather than guessed.
...
- **Page builders** - Elementor has dedicated clone-and-customize tools (plus Pro fonts/icons/code when Elementor Pro is active). Divi has structural read tools plus guarded whole-node replace/insert/delete for Divi 4 shortcodes and Divi 5 blocks. Beaver Builder, Bricks, Gutenberg, Spectra, and Stackable remain reachable through standard post or block content, but builder-specific storage is opaque unless covered by a dedicated integration.
- **Multilingual** - TranslatePress, Polylang, and WPML have dedicated read-only language tools (list languages, resolve a post's or term's language, list translation siblings). Translated posts also appear as separate posts and can be read or written via the standard post tools.
...
No. WordPress 6.9 added the Abilities API — a primitive for registering AI-callable functions — and the `wordpress/mcp-adapter` package bridges abilities to MCP. More MCP remains a complete server with its own authenticated Streamable HTTP transport, OAuth 2.0 connector flow, per-IP rate limiting, audit logging, sensitive-data redaction, 111 always-registered WordPress-core, Gutenberg, and webhook tools, and more tools from integrations you enable.

### Does More MCP work with WooCommerce?

Yes. When WooCommerce is active **and** you enable the WooCommerce integration under More MCP → Settings → Access → Plugins, More MCP adds 29 MCP tools spanning product management (simple and variable, including variation CRUD and global attribute management), full coupon management (list/get/create/update/delete + bulk trash purge), order management (view, update status), customer data, and store statistics. The integration is off until you enable it — activating WooCommerce alone does not expose the tools.
