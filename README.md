...
<p align="center"><strong>Securely connect AI assistants to WordPress through MCP.</strong><br>Self-hosted OAuth and API-key authentication, audit logs, safe writes, and 101–176 tools.</p>

More MCP turns WordPress into a secure, self-hosted [Model Context Protocol](https://modelcontextprotocol.io/) server. It lets Claude, ChatGPT, Gemini, AI coding tools, and other MCP-compatible agents work with your site through structured tools instead of screen scraping, browser automation, or copied-and-pasted content.

The plugin combines broad WordPress automation with controls designed for production sites. Every connection is authenticated through OAuth 2.0 or an API key, requests are rate-limited, WordPress capabilities are enforced inside each tool handler, and interactions are recorded without logging sensitive argument values. Credentials, tokens, and session state remain in your WordPress database; More MCP has no license server, telemetry, or vendor callback.

A standard installation exposes 101 tools for WordPress core and Gutenberg. Compatible plugins add specialized tools automatically, while ten high-impact plugin and theme lifecycle tools remain disabled until an administrator explicitly enables them. Depending on the active integrations and permissions, a site can expose between 101 and 176 tools through the native MCP endpoint, the classic REST API, and the WordPress Abilities API on WordPress 6.9 or later.
...
- [Tool inventory](#tool-inventory-101176-tools)
...
| Tool count | 101 always registered; 101–176 depending on integrations and permissions |
...
## Tool inventory: 101–176 tools

Every installation registers **101 tools**: 82 WordPress core tools and 19 Gutenberg tools. Eight detected integrations can add 65 tools, and ten plugin/theme lifecycle tools appear only when an administrator enables them. A site therefore exposes **101–176 tools**, depending on its active plugins and permissions.
...
Security. Most MCP plugins — and 41% of all public MCP servers — have no authentication at all. More MCP requires an API key for every session, rate-limits requests to prevent abuse, logs every interaction for audit purposes, and filters sensitive data (emails, PHP version, admin credentials) from responses.

### Does More MCP duplicate what WordPress core now does?

No. WordPress 6.9 added the Abilities API — a primitive for registering AI-callable functions — and the `wordpress/mcp-adapter` package bridges abilities to MCP. More MCP remains a complete server with its own authenticated Streamable HTTP transport, OAuth 2.0 connector flow, per-IP rate limiting, audit logging, sensitive-data redaction, 101 always-registered WordPress and Gutenberg tools, and up to 65 tools from detected integrations.
