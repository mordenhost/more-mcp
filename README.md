...
<p align="center"><strong>Give AI assistants a safe, authenticated way into WordPress.</strong><br>Self-hosted OAuth and API keys, per-tool capability checks, audit logging, and 107 always-on tools plus opt-in integrations.</p>
...
| Tool count | 107 always registered; 10 opt-in lifecycle tools; plus opt-in integration tools (live count shown in the Documentation panel) |
...
Every installation registers **107 always-on tools**: 85 WordPress core tools, 19 Gutenberg block tools, and 3 outbound-webhook tools (none are gated on a third-party plugin). Opt-in integrations add more tools once an administrator enables them (WooCommerce alone adds 29), and ten plugin/theme lifecycle tools appear only when an administrator enables them. Exact live counts are shown in the Documentation panel, which reads the registry directly.
...
No. WordPress 6.9 added the Abilities API — a primitive for registering AI-callable functions — and the `wordpress/mcp-adapter` package bridges abilities to MCP. More MCP remains a complete server with its own authenticated Streamable HTTP transport, OAuth 2.0 connector flow, per-IP rate limiting, audit logging, sensitive-data redaction, 107 always-registered WordPress-core, Gutenberg, and webhook tools, and more tools from integrations you enable.
